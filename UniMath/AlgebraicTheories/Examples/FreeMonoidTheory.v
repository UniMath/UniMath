...
Definition free_monoid_theory_data : algebraic_theory_data.
Proof.
...
Lemma free_monoid_is_theory : is_algebraic_theory free_monoid_theory_data.
Proof.
...
Proof.
  split;
    ...
    now rewrite (idpath _ : op (X := free_monoid_theory_algebra_to_setwithbinop A) _ _
      ...
      <- (pr_action _ (make_stn 1 0 (idpath _)) (λ _, x)),
...
Proof.
  use make_is_algebra.
  - intros m n f g a.
    unfold AlgebraCategory.comp_action_ax.
...
Proof.
  ...
  - intros n f.
    ...
    + intros i xs Haction.
      ...
      : free_monoid_theory 2) • (weqvecfun _ [((free_monoid_unit i) ; xs)])).
