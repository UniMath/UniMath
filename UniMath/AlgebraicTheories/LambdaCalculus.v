...
Definition lambda_calculus_ind
  ...
  : (∏ n l, A n l)
  := pr22 (pr222 (pr222 (pr222 L))) A f_var f_app f_abs f_subst f_subst_var f_subst_app f_subst_abs f_subst_subst f_beta f_eta.
...
Proof.
  use (lambda_calculus_ind
    ...
    (λ _ _ _, f_app _)
    (λ _ _, f_abs _)
    (λ _ _ _ _, f_subst _ _)
  );
    intros;
    use PathOverConstant_map1.
  - apply f_subst_var.
  - apply f_subst_app.
  - refine (f_subst_abs _ _ _ _ @ _).
    do 2 apply maponpaths.
    symmetry.
    apply extend_tuple_dep_const.
  - apply f_subst_subst.
  - refine (f_beta _ _ _ @ _).
    apply maponpaths.
    symmetry.
    apply extend_tuple_dep_const.
  - apply f_eta.
