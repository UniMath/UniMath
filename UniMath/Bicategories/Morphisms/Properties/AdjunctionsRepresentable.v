...
Definition left_adjoint_repr
           ...
           (l : x --> y)
  : UU
  := ∏ (w : B), is_right_adjoint (pre_comp w l).
...
Section LeftAdjointToLeftAdjointRepr.
  ...
  Section AdjointRepr.
    ...
    Definition left_adjoint_to_repr_left_adjoint_data
      : functor_data (hom x w) (hom y w).
    Proof.
    ...
    Definition left_adjoint_to_repr_left_adjoint_is_functor
      : is_functor left_adjoint_to_repr_left_adjoint_data.
    Proof.
      split.
      - intro f.
        apply lwhisker_id2.
    ...
    Definition left_adjoint_to_repr_left_adjoint
      : hom x w ⟶ hom y w.
    Proof.
      use make_functor.
      - exact left_adjoint_to_repr_left_adjoint_data.
      - exact left_adjoint_to_repr_left_adjoint_is_functor.
    Defined.

    Definition left_adjoint_to_repr_unit
      : functor_identity (hom x w)
        ⟹
        left_adjoint_to_repr_left_adjoint ∙ pre_comp w l.
    Proof.
      use make_nat_trans.
      - exact (λ f, linvunitor _ • (η ▹ _) • rassociator _ _ _).
      - abstract
          (intros f₁ f₂ α ; cbn ;
           rewrite !vassocr ;
    ...
    Definition left_adjoint_to_repr_counit
      : pre_comp w l ∙ left_adjoint_to_repr_left_adjoint
        ⟹
        functor_identity (hom y w).
    Proof.
      use make_nat_trans.
      - exact (λ f, lassociator _ _ _ • (ε ▹ _) • lunitor _).
      - abstract
          (intros f₁ f₂ α ; cbn ;
           rewrite !vassocr ;
           rewrite lwhisker_lwhisker ;
  ...
  Definition left_adjoint_to_left_adjoint_repr_form_adjunction
             (w : B)
    : form_adjunction
        (left_adjoint_to_repr_left_adjoint w)
        (pre_comp w l)
        (left_adjoint_to_repr_unit w)
        (left_adjoint_to_repr_counit w).
  Proof.
  ...
  Definition left_adjoint_to_left_adjoint_repr_are_adjoints
             (w : B)
    : are_adjoints (left_adjoint_to_repr_left_adjoint w) (pre_comp w l).
  Proof.
    use make_are_adjoints.
    - exact (left_adjoint_to_repr_unit w).
    - exact (left_adjoint_to_repr_counit w).
  ...
  Definition left_adjoint_to_adjunction_cat
             (w : B)
             (HB_2_1 : is_univalent_2_1 B)
    : @adjunction bicat_of_univ_cats (univ_hom HB_2_1 x w) (univ_hom HB_2_1 y w).
  Proof.
    refine (left_adjoint_to_repr_left_adjoint w
            ,,
            ((pre_comp w l
            ,,
            (left_adjoint_to_repr_unit w
...
Definition left_adjoint_equivalence_repr
           ...
           (l : x --> y)
  : UU
  := ∏ (w : B), adj_equivalence_of_cats (pre_comp w l).
...
Section LeftAdjointToLeftAdjointRepr.
  ...
  Section ToEquivalence.
    ...
    Let L : hom y w ⟶ hom x w := pre_comp w l.
    Let R : hom x w ⟶ hom y w := left_adjoint_to_repr_left_adjoint l Hl w.

    Definition left_adjequiv_to_left_adjequiv_repr_equiv_unit
      : functor_identity _ ⟹ L ∙ R.
    Proof.
      use make_nat_trans.
      - exact (λ f, linvunitor _ • (ε^-1 ▹ _) • rassociator _ _ _).
      - abstract
          (intros f₁ f₂ α ; cbn ;
           rewrite !vassocl ;
           rewrite lwhisker_lwhisker_rassociator ;
    ...
    Definition left_adjequiv_to_left_adjequiv_repr_equiv_counit
      : R ∙ L ⟹ functor_identity (hom x w).
    Proof.
      use make_nat_trans.
      - exact (λ f, lassociator _ _ _ • (η^-1 ▹ _) • lunitor _).
      - abstract
          (intros f₁ f₂ α ; cbn ;
           rewrite !vassocr ;
           rewrite lwhisker_lwhisker ;
    ...
    Definition left_adjequiv_to_left_adjequiv_repr_equiv
      : equivalence_of_cats (hom y w) (hom x w).
    Proof.
      use make_equivalence_of_cats.
      - use make_adjunction_data.
        + exact L.
        + exact R.
        + exact left_adjequiv_to_left_adjequiv_repr_equiv_unit.
        + exact left_adjequiv_to_left_adjequiv_repr_equiv_counit.
      - split.
        + intro f.
          apply is_inv2cell_to_is_z_iso ; cbn.
          is_iso.
...
Section LeftAdjointReprToLeftAdjoint.
  ...
  Let r : y --> x := right_adjoint (Hl x) (id₁ x).
  Let ε : invertible_2cell (l · r) (id₁ _)
    := nat_z_iso_pointwise_z_iso
         (counit_nat_z_iso_from_adj_equivalence_of_cats
            (Hl x))
         (id₁ x).

  Definition left_adjoint_repr_to_left_adjoint_unit
    : invertible_2cell (r · l) (id₁ y).
  Proof.
    apply z_iso_to_inv2cell.
    refine (z_iso_comp
              (nat_z_iso_pointwise_z_iso
  ...
  Definition left_adjoint_repr_to_left_adjoint_data
    : left_adjoint_data l
    := r ,, (ε^-1 ,, pr1 left_adjoint_repr_to_left_adjoint_unit).
  ...
  Proof.
    ...
    - cbn.
      is_iso.
    - exact (pr2 left_adjoint_repr_to_left_adjoint_unit).
  Defined.
