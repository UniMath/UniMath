...
Section FiberOfFibs.
  ...
  Definition comp_fib_fiber
             {D₁ D₂ : strict_fiber_bicat
                        disp_bicat_of_fibs
                        cleaving_of_fibs_local_iso_cleaving
                        C}
             {F G H : D₁ --> D₂}
             (α : F ==> G)
             (β : G ==> H)
    ...
    : pr11 (α • β) x xx
      ...
      transportf
        (λ z, _ -->[ z ] _)
        (id_left _)
        (pr11 α x xx ;; pr11 β x xx)%mor_disp.
  Proof.
    cbn.
    ...
    etrans.
    {
      exact (disp_nat_trans_transportf
               ...
               (@id2_left
                  ...
                  _)
               _ _
               (disp_nat_trans_comp (pr1 α) (pr1 β))
               x xx).
    }
    cbn.
    apply maponpaths_2.
  ...
  Definition lunitor_fib_fiber
    ...
    : pr11 (lunitor F) x xx
      =
      pr2 D₂ x x (id₁ x) (pr11 F x xx).
  Proof.
  Admitted.

  Definition runitor_fib_fiber
             {D₁ D₂ : strict_fiber_bicat
                        disp_bicat_of_fibs
                        cleaving_of_fibs_local_iso_cleaving
                        C}
  ...
  Definition lassociator_fib_fiber
             {D₁ D₂ D₃ D₄ : strict_fiber_bicat
                              disp_bicat_of_fibs
                              cleaving_of_fibs_local_iso_cleaving
                              C}
             (F : D₁ --> D₂)
    ...
    : UU.
  Proof.
    refine (pr11 (lassociator F G H) x xx
            =
            _).
    cbn.
  Admitted.

  Definition lwhisker_fib_fiber
             {D₁ D₂ D₃ : strict_fiber_bicat
                           disp_bicat_of_fibs
                           cleaving_of_fibs_local_iso_cleaving
                           C}
             (F : D₁ --> D₂)
  ...
  Definition rwhisker_fib_fiber
    ...
    : UU.
  Proof.
    refine (pr11 (α ▹ G) x xx
            =
            _).
  Admitted.

  Definition fibs_to_fiber_data
    : psfunctor_data
        (fib_slice_bicat C)
        (strict_fiber_bicat
           disp_bicat_of_fibs
  ...
  Proof.
    ...
    - exact (λ D, D).
    - exact (λ D₁ D₂ FF, FF).
    - exact (λ D₁ D₂ FF GG α, α ,, tt).
    - exact (λ D, disp_nat_trans_id _ ,, tt).
    - cbn.
      refine (λ D₁ D₂ D₃ FF GG, _ ,, tt).
      simple refine (_ ,, _).
      + intros x xx.
        exact (pr12 (pr2 D₃ x x (id₁ x) (pr1 GG x (pr1 FF x xx)))).
      + intros x y f xx yy ff.
        apply cartesian_factorisation_commutes.
  Defined.

  Definition fibs_to_fiber_laws
    : psfunctor_laws fibs_to_fiber_data.
  Proof.
    repeat split.
    - intros D₁ D₂ FF GG HH α β.
      use subtypePath.
      {
        intro.
        apply isapropunit.
    ...
    - intros D₁ D₂ F.
      ...
      etrans.
      {
        exact (lunitor_fib_fiber (# fibs_to_fiber_data F) xx).
      }
      refine (!_).
      refine (comp_fib_fiber _ _ _ @ _).
      etrans.
      {
        apply maponpaths.
        apply maponpaths_2.
        apply comp_fib_fiber.
      }
      cbn.
      cbn.
      refine (lunitor_fib_fiber _ _ @ _).
      rewrite lunitor_fib_fiber.
  ...
  Proof.
    use make_psfunctor.
    - exact fibs_to_fiber_data.
    Check cleaving_of_fibs_local_iso_cleaving.
    simple refine ().
    Check disp_bicat_of_fibs.
