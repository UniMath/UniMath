...
Section PullbackOfSFib.
  ...
  Definition test
             ...
             {h₁ h₂ : z --> a}
             (β : h₁ ==> h₂)
             (Hβ : is_cartesian_2cell_sfib f (β ▹ p₁))
    : is_cartesian_2cell_sfib p₂ β.
  Proof.
    intros k δ δp q.
    use iscontraprop1.
    - admit.
    - pose (is_cartesian_2cell_sfib_factor
              _
              Hβ
              (δ ▹ p₁)
              (rassociator _ _ _
      ...
      + (*
        assert ((δ ▹ p₁) ▹ f =
     ((((((rassociator k p₁ f • (k ◃ γ)) • lassociator k p₂ g) • (δp ▹ g))
        • rassociator h₁ p₂ g) • (h₁ ◃ γ ^-1)) • lassociator h₁ p₁ f) •
                                                                      ((β ▹ p₁) ▹ f))
          as X.
        {
          rewrite !vassocl.
  ...
  Section Cleaving.
    ...
    Local Definition cleaving_cone
      : pb_cone f g.
    Proof.
      use make_pb_cone.
      - exact x.
    ...
    Definition pb_of_sfib_cleaving_mor
      : x --> a
      := pb_ump_mor H cleaving_cone.

    Definition pb_of_sfib_cleaving_cell
      : pb_of_sfib_cleaving_mor ==> h₂.
    Proof.
      use (pb_ump_cell H).
      - exact (pb_ump_mor_pr1 H cleaving_cone • internal_sfib_cleaving_lift_cell _ _ _).
      - exact (pb_ump_mor_pr2 H cleaving_cone • α).
      - (*cbn.
        rewrite <- !rwhisker_vcomp.
        rewrite internal_sfib_cleaving_over.
        rewrite !vassocl.
        ...
        do 2 apply maponpaths_2.
        rewrite !vassocl.
        Check internal_sfib_cleaving_com.
        etrans.
        {
          do 2 apply maponpaths.
        ...
        unfold pb_of_sfib_cleaving_mor.
        cbn.
        rewrite !vassocl.
          apply maponpaths_2.
        rewrite
    ...
    Proof.
      exact (pb_ump_mor_pr2 H cleaving_cone).
    Defined.
  ...
  Proof.
    ...
    apply test.
    unfold pb_of_sfib_cleaving_cell.
    rewrite (pb_ump_cell_pr1 H).
    use vcomp_is_cartesian_2cell_sfib.
    - apply invertible_is_cartesian_2cell_sfib.
      apply (pb_ump_mor_pr1 H (cleaving_cone α)).
    - apply internal_sfib_cleaving_is_cartesian.
  Defined.
...
Section StreetFibIsInternalStreetFib.
  ...
  Section InternalCleaving.
    ...
    Definition street_fib_is_internal_over_data
      : nat_trans_data (street_fib_is_internal_lift · P : _ ⟶ _) (pr1 F)
