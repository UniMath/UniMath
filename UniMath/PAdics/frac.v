...
Definition afldfracapartrelpre : hrel ( dirprod A (
aintdomazerosubmonoid A ) ) := fun ab cd : _ => ( ( pr1 ab ) * ( pr1 (
pr2 cd ) ) ) # ( ( pr1 cd ) * ( pr1 ( pr2 ab ) ) ).

Lemma afldfracapartiscomprel : iscomprelrel ( eqrelcommrngfrac A (
aintdomazerosubmonoid A ) ) ( afldfracapartrelpre ).  Proof.  intros
ab cd ef gh p q. unfold afldfracapartrelpre.  destruct ab as [ a b
...
Lemma issymmafldfracapartrelpre : issymm afldfracapartrelpre.  Proof.
...
Lemma isapartafldfracapartrel : isapart afldfracapartrel.  Proof.
...
(afldfracapart) ( commrngfracop1 A ( aintdomazerosubmonoid A ) a b) (
...
afldfracapartrelpre.  simpl.
  ...
  (pr1aintdom A))) f a) b) d) ) as i.  permute. rewrite i in u. assert
  ...
  (acommrngtocommrng (pr1aintdom A)))) (@op2 (@pr1 setwith2binop (fun
  X : setwith2binop => @iscommrngops (pr1setwith2binop X) (@op1 X)
  (@op2 X)) (acommrngtocommrng (pr1aintdom A))) c f) b) b) ) as
  j. permute. rewrite j in u.  assert ( (@op2 (pr1rng (commrngtorng
  (acommrngtocommrng (pr1aintdom A)))) (@op2 (pr1rng (commrngtorng
  (acommrngtocommrng (pr1aintdom A)))) (@op2 (@pr1 setwith2binop (fun
  X : setwith2binop => @iscommrngops (pr1setwith2binop X) (@op1 X)
  (@op2 X)) (acommrngtocommrng (pr1aintdom A))) b e) b) d) ~> (@op2
  (pr1rng (commrngtorng (acommrngtocommrng (pr1aintdom A)))) (@op2
  (pr1rng (commrngtorng (acommrngtocommrng (pr1aintdom A)))) (@op2
...
Proof.  intros. unfold isbinopapartl.  assert ( forall a b c :
commrngfrac A ( aintdomazerosubmonoid A ), isaprop ( pr1
(afldfracapart ) ( commrngfracop2 A ( aintdomazerosubmonoid A ) a b) (
...
fun a b c => hProppair _ ( int a b c ) ) ).  intros ab cd ef p.
  ...
  ) ( tpair f f' ) ) ) ) as u. apply p.  unfold afldfracapart in
  *. simpl. unfold afldfracapartrel.  unfold quotrel. rewrite (
  ...
  setwith2binop (fun X : setwith2binop => @iscommrngops
  (pr1setwith2binop X) (@op1 X) (@op2 X)) (acommrngtocommrng
  (pr1aintdom A))) b f)) ) as i. change ( c * f * a * b ~> a * c * ( b
...
Definition afldfrac0 : acommrng.  Proof.  intros. split with (
commrngfrac A ( aintdomazerosubmonoid A ) ).  split with (
afldfracapart ). split.  split. apply ( isbinapartlafldfracop1
). apply ( isbinapartrafldfracop1 ).  split. apply (
isbinapartlafldfracop2 ). apply ( isbinapartrafldfracop2 ).  Defined.

Definition afldfracmultinvint ( ab : dirprod A ( aintdomazerosubmonoid
A ) ) ( is : afldfracapartrelpre ab ( dirprodpair ( @rngunel1 A ) (
...
a. unfold afldfracapartrelpre in is. simpl in is.  change ( a # 0
). rewrite ( @rngmult0x A ) in is. rewrite ( @rngrunax2 A ) in
is. assumption.  Defined.

Definition afldfracmultinv ( a : afldfrac0 ) ( is : a # 0 ) :
multinvpair afldfrac0 a.  Proof.  intros. assert ( forall b :
afldfrac0, isaprop ( b # 0 -> multinvpair afldfrac0 b ) ) as int.
intros. apply impred. intro p. apply ( isapropmultinvpair afldfrac0 ).
assert ( forall b : afldfrac0, b # 0 -> multinvpair afldfrac0 b ) as
p.  apply ( setquotunivprop _ ( fun x0 => hProppair _ ( int x0 ) ) ).
intros bc q.  destruct bc as [ b c ].  assert ( afldfracapartrelpre (
...
afldfracmultinvint ( dirprodpair b c ) is' ) ).
  ...
  aintdomazerosubmonoid A ) ( pr2 ( afldfracmultinvint ( dirprodpair b
 ...
 ) ( dirprodpair ( @op2 A b ( pr1 ( afldfracmultinvint ( dirprodpair b
