...
Definition Dcuts_def_bot (X : subtype NonnegativeRationals) : UU :=
  Π x : NonnegativeRationals, X x ->
    Π y : NonnegativeRationals, y <= x -> X y.
Definition Dcuts_def_open (X : subtype NonnegativeRationals) : UU :=
  Π x : NonnegativeRationals, X x ->
    hexists (fun y : NonnegativeRationals => (X y) × (x < y)).
Definition Dcuts_def_finite (X : subtype NonnegativeRationals) : hProp :=
  ∃ ub : NonnegativeRationals, ¬ (X ub).
Definition Dcuts_def_error (X : subtype NonnegativeRationals) : UU :=
  Π r : NonnegativeRationals, 0 < r -> (¬ (X r)) ∨ Σ q : NonnegativeRationals, (X q) × (¬ (X (q + r))).
Definition Dcuts_def_error' (X : subtype NonnegativeRationals) (k : NonnegativeRationals) : UU :=
  Π r, 0 < r -> r <= k -> (¬ (X r)) ∨ Σ q : NonnegativeRationals, (0 < q) × (X q) × (¬ (X (q + r))).

Lemma Dcuts_def_error'_correct1 (X : subtype NonnegativeRationals) (k : NonnegativeRationals) :
...
Qed.
Lemma Dcuts_def_error'_correct2 (X : subtype NonnegativeRationals) (k : NonnegativeRationals) :
...
Lemma Dcuts_def_error_finite (X : subtype NonnegativeRationals) :
...
Lemma Dcuts_def_error_not_empty (X : subtype NonnegativeRationals) :
...
Lemma isaprop_Dcuts_def_bot (X : subtype NonnegativeRationals) : isaprop (Dcuts_def_bot X).
Proof.
  intros X.
  repeat (apply impred_isaprop ; intro).
  now apply pr2.
Qed.
Lemma isaprop_Dcuts_def_open (X : subtype NonnegativeRationals) : isaprop (Dcuts_def_open X).
Proof.
  intros X.
  repeat (apply impred_isaprop ; intro).
  now apply pr2.
Qed.
Lemma isaprop_Dcuts_def_error (X : subtype NonnegativeRationals) : isaprop (Dcuts_def_error X).
Proof.
  intros X.
  repeat (apply impred_isaprop ; intro).
  now apply pr2.
...
Lemma isaprop_Dcuts_subtype (X : subtype NonnegativeRationals) :
...
Definition Dcuts_subtype : subtype (subtype NonnegativeRationals) :=
  fun X : subtype NonnegativeRationals => hProppair _ (isaprop_Dcuts_subtype X).
...
Definition pr1Dcuts (x : Dcuts_set) : subtype NonnegativeRationals := pr1 x.
...
Definition Dcuts_NQmult_val : subtype NonnegativeRationals :=
...
Definition Dcuts_mult_val : subtype NonnegativeRationals :=
...
Definition Dcuts_inv_val : subtype NonnegativeRationals :=
...
Proof.
  ...
  - apply hinhuniv ; intros [|] ; apply hinhuniv ;
    [ intros [|]
    | intros ((rzx,rzy),(Hr,(p,q))) ; generalize p q; clear p q; simpl in Hr ].
    + apply hinhfun ; intros ((rz,rx),p) ; generalize p; clear p; simpl ; intros (Hr,(Zr,XYr)).
      exists (rz,rx) ; simpl ; repeat split.
      * exact Hr.
      * exact Zr.
      * apply hinhpr ; left.
        now apply hinhpr ; left.
    + apply hinhfun ; intros ((rz,ry),p) ; generalize p; clear p; simpl ; intros (Hr,(Zr,Yr)).
...
Proof.
  ...
  - generalize (is_Dcuts_open _ _ X0) ; apply hinhuniv ; intros (x,(Xx,Hx0)).
    ...
    + unfold neg ; apply (hinhuniv (P := hProppair _ isapropempty)) ; intros ((ry,rx),p); generalize p; clear p; simpl ; intros (Hr',(Yr,Xr)).
      revert Hr' ; apply gtNonnegativeRationals_noteq.
      apply multNonnegativeRationals_ltcompat.
      now apply (Dcuts_finite Y).
      apply (Dcuts_finite X).
...
Proof.
  ...
  apply Dcuts_eq_is_eq.
  intros r ; split.
  - apply hinhuniv ; intros [ | ] ; apply hinhfun ; [intros [Xr | Xr] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (->,(Xr,Yr))].
    + exists (r / 2)%NRat.
      simpl ; split.
...
Definition Dcuts_minus_val : subtype NonnegativeRationals :=
...
Proof.
  ...
  - apply hinhuniv ; intros (q,(YZq,Hq)).
    revert YZq ; apply hinhuniv ; intros [ | ] ; apply hinhuniv ; [intros [ Yq | Zq ] | intros ((qy,qz),p) ; generalize p; clear p; simpl ; intros (H,(Yq,Zq)) ].
  ...
  - apply hinhuniv ; intros (x,(nYx,Xx)).
    ...
    + apply hinhpr.
      ...
      * rewrite minusNonnegativeRationals_zero_r.
        ...
        now apply isnonnegative_NonnegativeRationals.
        exact Hx'.
  - apply hinhuniv ; intros (r,(_,p)); generalize p; clear p.
    apply hinhfun ; intros (x,(Xx,Hx)).
    exists x ; split.
...
Definition Dcuts_max_val : subtype NonnegativeRationals :=
...
Proof.
  ...
  intros r ; split.
  - apply hinhuniv.
    intros ((rz,rxy),p) ; generalize p; clear p; simpl fst ; simpl snd ; intros (->,(Zr,p)) ; generalize p; clear p.
    apply hinhfun.
    intros [Xr | Yr].
    + left.
      apply hinhpr.
  ...
  - apply hinhuniv.
    intros [ | ].
    + apply hinhfun.
      intros ((rz,rx),p) ; generalize p; clear p; simpl fst ; simpl snd ; intros (->,(Zr,Xr)).
      exists (rz,rx).
      repeat split.
      exact Zr.
      apply hinhpr.
      now left.
    + apply hinhfun.
      intros ((rz,ry),p) ; generalize p; clear p; simpl fst ; simpl snd ; intros (->,(Zr,Yr)).
      exists (rz,ry).
      repeat split.
      exact Zr.
      apply hinhpr.
  ...
  - apply hinhuniv ; intros [ | ] ; apply hinhuniv ; intros [ | ].
    + apply hinhfun.
      ...
      * apply hinhpr.
        ...
        apply hinhpr.
        now left.
    + apply hinhfun ; intros ((rz,rx),p) ; generalize p; clear p; simpl fst ; simpl snd ; intros (->,(Zr,Xr)).
      right.
      apply hinhpr.
      exists (rz,, rx).
      repeat split.
    ...
    + apply hinhfun.
      ...
      * apply hinhpr.
        ...
        apply hinhpr.
        now right.
    + apply hinhfun ; intros ((rz,ry),p) ; generalize p; clear p; simpl fst ; simpl snd ; intros (->,(Zr,Yr)).
      right.
      apply hinhpr.
      exists (rz,, ry).
      repeat split.
...
Proof.
  ...
  - intros r.
    ...
    exact H0.
    apply hinhuniv.
    intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (->,(Xr,Yr)).
    apply fromempty.
...
Definition Dcuts_half_val : subtype NonnegativeRationals :=
  λ r, X (r + r).
...
Proof.
  ...
  apply Dcuts_eq_is_eq.
  intros r ; split.
  - apply hinhuniv ; intros [ | ] ; apply hinhfun ; [intros [Xr | Yr] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (Hr,(Xr,Yr))].
    + left.
      apply hinhpr.
      left.
      exact Xr.
    ...
    + right.
      ...
      * rewrite <- NQhalf_double.
        exact Yr.
  - apply hinhuniv ; intros [ | ] ; apply hinhfun ; [intros [Xr | Yr] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (->,(Xr,Yr))].
    + left.
      apply hinhpr.
      left.
      exact Xr.
...
Proof.
  ...
  - intros Hr.
    ...
    now repeat split.
  - apply hinhuniv ; intros [|] ; apply hinhuniv ; [intros [ | ] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (Hr,(Xr,Yr)) ].
...
Context (U : nat -> subtype NonnegativeRationals)
...
Definition Dcuts_lim_cauchy_val : subtype NonnegativeRationals :=
...
Proof.
  ...
  - apply hinhpr ; left.
    ...
    + specialize (HuN' _ (isreflnatleh _)).
      ...
      apply hinhuniv ; intros [ | ] ; apply hinhuniv ; [ intros [H | H] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (Hr,(Xr,Yr))].
  ...
  - destruct HuN as (q,(UNq,nUNq)).
    ...
    +  apply hinhpr ; right.
      ...
      * apply hinhpr.
        ...
        apply hinhuniv ; intros [ | ] ; apply hinhuniv ; [intros [Xr | Yr] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (Hr,(Xr,Yr))].
        { apply (U_bot n) with (1 := Xr).
          pattern q at 2 ;
  ...
  - eapply istrans_Dcuts_lt_le_rel.
    ...
    + intros r Hr.
      ...
      * apply notlt_geNonnegativeRationals in Hrc.
        ...
        apply hinhuniv ; intros [|] ; apply hinhuniv ; [ intros [Xr | Yr] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (->,(Xr,Yr))].
        apply is_Dcuts_bot with (1 := Xr).
        now apply minusNonnegativeRationals_le.
        simpl in Yr.
...
Qed.
Lemma Dcuts_of_Dcuts_bij' :
  Π E : subtype Dcuts, Π (E_bot : Π x : Dcuts, E x -> Π y : Dcuts, y <= x -> E y) (E_open : Π x : Dcuts, E x -> ∃ y : Dcuts, x < y × E y),
...
Proof.
  ...
  - apply hinhuniv.
    simpl pr1.
    intros (r,(Hr,p)); generalize p; clear p.
    apply hinhuniv.
    intros X.
...
Lemma isub_Dcuts_of_Dcuts (E : subtype Dcuts) E_bot E_error :
...
Proof.
  ...
  now exists x.
Qed.
Lemma islbub_Dcuts_of_Dcuts (E : subtype Dcuts) E_bot E_error :
...
Qed.
Lemma islub_Dcuts_of_Dcuts (E : subtype eo_Dcuts) E_bot E_error :
...
Definition NonnegativeReals_to_hsubtypesNonnegativeRationals :
  NonnegativeReals → (subtype NonnegativeRationals) := pr1.
  ...
  - intros Hr.
    ...
    + assert (Hxy : (0 < x + y)%NRat).
      ...
      * unfold divNonnegativeRationals.
        ...
        exact Hr.
        exact Hxy.
  - apply hinhuniv ; intros [ | ] ; apply hinhuniv ; [intros [Hrx | Hry] | intros ((rx,ry),p) ; generalize p; clear p; simpl ; intros (->,(Hrx,Hry))] ; simpl.
