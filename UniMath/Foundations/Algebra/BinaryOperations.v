...
Definition islcancelable {X : UU} (opp : binop X) (x : X) : UU
  := isincl (fun x0 : X => opp x x0).

Definition isrcancelable {X : UU} (opp : binop X) (x : X) : UU
  := isincl (fun x0 : X => opp x0 x).

Definition iscancelable {X : UU} (opp : binop X) (x : X) : UU
  := dirprod (islcancelable opp x) (isrcancelable opp x).

Definition islinvertible {X : UU} (opp : binop X) (x : X) : UU
  := isweq (fun x0 : X => opp x x0).

Definition isrinvertible {X : UU} (opp : binop X) (x : X) : UU
  := isweq (fun x0 : X => opp x0 x).
...
Definition islunit {X : hSet} (opp : binop X) (un0 : X) : UU
  := Π x : X, paths (opp un0 x) x.

Lemma isapropislunit {X : hSet} (opp : binop X) (un0 : X) :
  isaprop (islunit opp un0).
Proof.
  intros.
  apply impred. intro x.
  simpl. apply (setproperty X).
...
Definition isrunit {X : hSet} (opp : binop X) (un0 : X) : UU
  := Π x : X, paths (opp x un0) x.

Lemma isapropisrunit {X : hSet} (opp : binop X) (un0 : X) :
  isaprop (isrunit opp un0).
Proof.
  intros.
  apply impred. intro x.
  simpl. apply (setproperty X).
...
Definition isunital {X : hSet} (opp : binop X) : UU
  := total2 (fun un0 : X => isunit opp un0).
Definition isunitalpair {X : hSet} {opp : binop X} (un0 : X)
           (is : isunit opp un0) : isunital opp := tpair _ un0 is.

Lemma isapropisunital {X : hSet} (opp : binop X) : isaprop (isunital opp).
Proof.
  intros.
  apply (@isapropsubtype
           X (fun un0 : _ => hconj (hProppair _ (isapropislunit opp un0))
...
Definition ismonoidop {X : hSet} (opp : binop X) : UU
  := dirprod (isassoc opp) (isunital opp).
Definition assocax_is {X : hSet} {opp : binop X} :
  ismonoidop opp -> isassoc opp := @pr1 _ _.
Definition unel_is {X : hSet} {opp : binop X} (is : ismonoidop opp) : X
  := pr1 (pr2 is).
...
Definition isgroppair {X : hSet} {opp : binop X}
           (is1 : ismonoidop opp) (is2 : invstruct opp is1) : isgrop opp
  := tpair (fun is : ismonoidop opp => invstruct opp is) is1 is2.
Definition pr1isgrop (X : hSet) (opp : binop X) :
  isgrop opp -> ismonoidop opp := @pr1 _ _.
Coercion pr1isgrop : isgrop >-> ismonoidop.

Definition grinv_is {X : hSet} {opp : binop X} (is : isgrop opp) :
  X -> X := pr1 (pr2 is).
...
Lemma isgropif {X : hSet} {opp : binop X} (is0 : ismonoidop opp)
      (is : Π x : X, hexists (fun x0 : X => eqset (opp x x0) (unel_is is0))) :
  isgrop opp.
Proof.
...
Definition iscomm {X : hSet} (opp : binop X) : UU
  := Π x x' : X, paths (opp x x') (opp x' x).
...
Definition isabmonoidop {X : hSet} (opp : binop X) : UU
  := dirprod (ismonoidop opp) (iscomm opp).
Definition pr1isabmonoidop (X : hSet) (opp : binop X) :
  isabmonoidop opp -> ismonoidop opp := @pr1 _ _.
Coercion pr1isabmonoidop : isabmonoidop >-> ismonoidop.

Definition commax_is {X : hSet} {opp : binop X} (is : isabmonoidop opp) :
  iscomm opp := pr2 is.
...
Lemma abmonoidoprer {X : hSet} {opp : binop X} (is : isabmonoidop opp)
      (a b c d : X) : paths (opp (opp a b) (opp c d)) (opp (opp a c) (opp b d)).
Proof.
  intros.
...
Lemma weqlcancelablercancelable {X : hSet} (opp : binop X) (is : iscomm opp)
      (x : X) : weq (islcancelable opp x) (isrcancelable opp x).
Proof.
  intros.
...
Lemma weqlinvertiblerinvertible {X : hSet} (opp : binop X) (is : iscomm opp)
      (x : X) : weq (islinvertible opp x) (isrinvertible opp x).
Proof.
  intros.
...
Proof.
  ...
  split with f.
  apply (isweqimplimpl f g (isapropislunit opp un0)
                       (isapropisrunit opp un0)).
Defined.


Lemma weqlinvrinv {X : hSet} (opp : binop X) (is : iscomm opp) (un0 : X)
      (inv0 : X -> X) : weq (islinv opp un0 inv0) (isrinv opp un0 inv0).
Proof.
  intros.
  ...
  split with f.
  apply (isweqimplimpl f g (isapropislinv opp un0 inv0)
                       (isapropisrinv opp un0 inv0)).
Defined.
...
Definition isabgrop {X : hSet} (opp : binop X) : UU
  := dirprod (isgrop opp) (iscomm opp).
Definition pr1isabgrop (X : hSet) (opp : binop X) :
  isabgrop opp -> isgrop opp := @pr1 _ _.
Coercion pr1isabgrop : isabgrop >-> isgrop.

Definition isabgroptoisabmonoidop (X : hSet) (opp : binop X) :
  isabgrop opp -> isabmonoidop opp
...
Definition isldistr {X : hSet} (opp1 opp2 : binop X) : UU
  := Π x x' x'' : X, paths (opp2 x'' (opp1 x x'))
                           (opp1 (opp2 x'' x) (opp2 x'' x')).

Lemma isapropisldistr {X : hSet} (opp1 opp2 : binop X) :
  isaprop (isldistr opp1 opp2).
Proof.
  intros.
  apply impred. intro x.
  apply impred. intro x'.
...
Definition isrdistr {X : hSet} (opp1 opp2 : binop X) : UU
  := Π x x' x'' : X, paths (opp2 (opp1 x x') x'')
                           (opp1 (opp2 x x'') (opp2 x' x'')).

Lemma isapropisrdistr {X : hSet} (opp1 opp2 : binop X) :
  isaprop (isrdistr opp1 opp2).
Proof.
  intros.
  apply impred. intro x.
  apply impred. intro x'.
...
Lemma isapropisdistr {X : hSet} (opp1 opp2 : binop X) :
  isaprop (isdistr opp1 opp2).
Proof.
  intros.
  apply (isofhleveldirprod 1 _ _ (isapropisldistr _ _)
                           (isapropisrdistr _ _)).
Defined.
...
Proof.
  ...
  split with f.
  apply (isweqimplimpl f g (isapropisldistr opp1 opp2)
                       (isapropisrdistr opp1 opp2)).
Defined.
...
Definition isrigops {X : hSet} (opp1 opp2 : binop X) : UU
  := dirprod (total2 (fun (axs : dirprod (isabmonoidop opp1) (ismonoidop opp2)) =>
                        (dirprod (Π x : X, paths (opp2 (unel_is (pr1 axs)) x)
                                                 (unel_is (pr1 axs))))
                          (Π x : X, paths (opp2 x (unel_is (pr1 axs)))
                                          (unel_is (pr1 axs)))))
             (isdistr opp1 opp2).

Definition rigop1axs_is {X : hSet} {opp1 opp2 : binop X} :
  isrigops opp1 opp2 -> isabmonoidop opp1 := fun is : _ => pr1 (pr1 (pr1 is)).
Definition rigop2axs_is {X : hSet} {opp1 opp2 : binop X} :
  isrigops opp1 opp2 -> ismonoidop opp2 := fun is : _ => pr2 (pr1 (pr1 is)).
...
Definition rigunel1_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrigops opp1 opp2) : X := pr1 (pr2 (pr1 (rigop1axs_is is))).
Definition rigunel2_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrigops opp1 opp2) : X := (pr1 (pr2 (rigop2axs_is is))).
Definition rigmult0x_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrigops opp1 opp2) (x : X) :
  paths (opp2 (rigunel1_is is) x) (rigunel1_is is) := pr1 (pr2 (pr1 is)) x.
Definition rigmultx0_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrigops opp1 opp2) (x : X) :
  paths (opp2 x (rigunel1_is is)) (rigunel1_is is) := pr2 (pr2 (pr1 is)) x.


Lemma isapropisrigops {X : hSet} (opp1 opp2 : binop X) :
  isaprop (isrigops opp1 opp2).
Proof.
  intros.
  apply (isofhleveldirprod 1).
  - apply (isofhleveltotal2 1).
...
Definition rngop1axs_is {X : hSet} {opp1 opp2 : binop X} :
  isrngops opp1 opp2 -> isabgrop opp1 := fun is : _ => pr1 (pr1 is).
Definition rngop2axs_is {X : hSet} {opp1 opp2 : binop X} :
  isrngops opp1 opp2 -> ismonoidop opp2 := fun is : _ => pr2 (pr1 is).
...
Definition rngunel1_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) : X := unel_is (pr1 (pr1 is)).
Definition rngunel2_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) : X := unel_is (pr2 (pr1 is)).


Lemma isapropisrngops {X : hSet} (opp1 opp2 : binop X) :
  isaprop (isrngops opp1 opp2).
Proof.
  intros.
  apply (isofhleveldirprod 1).
  - apply (isofhleveldirprod 1).
...
Lemma multx0_is_l {X : hSet} {opp1 opp2 : binop X} (is1 : isgrop opp1)
      (is2 : ismonoidop opp2) (is12 : isdistr opp1 opp2) :
  Π x : X, paths (opp2 x (unel_is (pr1 is1))) (unel_is (pr1 is1)).
Proof.
  intros.
...
Lemma mult0x_is_l {X : hSet} {opp1 opp2 : binop X} (is1 : isgrop opp1)
      (is2 : ismonoidop opp2) (is12 : isdistr opp1 opp2) :
  Π x : X, paths (opp2 (unel_is (pr1 is1)) x) (unel_is (pr1 is1)).
Proof.
  intros.
  destruct is12 as [ ldistr0 rdistr0 ].
...
Lemma islinvmultwithminus1_is_l {X : hSet} {opp1 opp2 : binop X}
      ...
      (x : X) : paths (opp1 (opp2 (minus1_is_l is1 is2) x) x)
                      (unel_is (pr1 is1)).
Proof.
  intros.
...
Lemma isrinvmultwithminus1_is_l {X : hSet} {opp1 opp2 : binop X}
      (is1 : isgrop opp1) (is2 : ismonoidop opp2) (is12 : isdistr opp1 opp2)
      (x : X) : paths (opp1 x (opp2 (minus1_is_l is1 is2) x))
                      (unel_is (pr1 is1)).
Proof.
  intros.
  set (xinv := opp2 (minus1_is_l is1 is2) x).
...
Lemma isminusmultwithminus1_is_l {X : hSet} {opp1 opp2 : binop X}
      (is1 : isgrop opp1) (is2 : ismonoidop opp2) (is12 : isdistr opp1 opp2)
      (x : X) : paths (opp2 (minus1_is_l is1 is2) x) (grinv_is is1 x).
Proof.
  intros.
...
Lemma isrngopsif {X : hSet} {opp1 opp2 : binop X} (is1 : isgrop opp1)
      (is2 : ismonoidop opp2) (is12 : isdistr opp1 opp2) : isrngops opp1 opp2.
Proof.
  intros.
  ...
  - split.
    + split with is1.
      ...
      apply (invmaponpathsweq
               (weqpair _ (isweqrmultingr_is is1 (opp2 (minus1_is_l is1 is2)
                                                       (opp1 x y))))).
      simpl. rewrite (isrinvmultwithminus1_is_l is1 is2 is12 (opp1 x y)).
      rewrite (pr1 is12 x y _).
      destruct (assoc1 (opp1 y x) (opp2 (minus1_is_l is1 is2) x)
                       (opp2 (minus1_is_l is1 is2) y)).
...
Definition rngmultx0_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) :
  Π (x : X),
  opp2 x (unel_is (pr1 (rngop1axs_is is))) = unel_is (pr1 (rngop1axs_is is))
  := multx0_is_l (rngop1axs_is is) (rngop2axs_is is) (rngdistraxs_is is).

Definition rngmult0x_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) :
  Π (x : X),
  opp2 (unel_is (pr1 (rngop1axs_is is))) x = unel_is (pr1 (rngop1axs_is is))
  := mult0x_is_l (rngop1axs_is is) (rngop2axs_is is) (rngdistraxs_is is).

Definition rngminus1_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) : pr1hSet X
  := minus1_is_l (rngop1axs_is is) (rngop2axs_is is).

Definition rngmultwithminus1_is {X : hSet} {opp1 opp2 : binop X}
           (is : isrngops opp1 opp2) :
  Π (x : X),
  opp2 (minus1_is_l (rngop1axs_is is) (rngop2axs_is is)) x
  = grinv_is (rngop1axs_is is) x
  := isminusmultwithminus1_is_l
       (rngop1axs_is is) (rngop2axs_is is) (rngdistraxs_is is).

Definition isrngopstoisrigops (X : hSet) (opp1 opp2 : binop X)
           (is : isrngops opp1 opp2) : isrigops opp1 opp2.
Proof.
  intros. split.
  - split with (dirprodpair (isabgroptoisabmonoidop _ _ (rngop1axs_is is))
                            (rngop2axs_is is)).
    split.
...
Definition rigiscommop2_is {X : hSet} {opp1 opp2 : binop X}
           (is : iscommrigops opp1 opp2) : iscomm opp2 := pr2 is.

Lemma isapropiscommrig {X : hSet} (opp1 opp2 : binop X) :
  isaprop (iscommrigops opp1 opp2).
Proof.
  intros.
  apply (isofhleveldirprod 1).
...
Definition rngiscommop2_is {X : hSet} {opp1 opp2 : binop X}
           (is : iscommrngops opp1 opp2) : iscomm opp2 := pr2 is.

Lemma isapropiscommrng {X : hSet} (opp1 opp2 : binop X) :
  isaprop (iscommrngops opp1 opp2).
Proof.
  intros.
  apply (isofhleveldirprod 1).
...
Lemma isapropisbinopfun {X Y : setwithbinop} (f : X -> Y) :
  isaprop (isbinopfun f).
Proof.
  intros.
  apply impred. intro x.
  apply impred. intro x'.
...
Definition binopfun (X Y : setwithbinop) : UU
  := total2 (fun f : X -> Y => isbinopfun f).
Definition binopfunpair {X Y : setwithbinop} (f : X -> Y) (is : isbinopfun f) :
  binopfun X Y := tpair _ f is.
Definition pr1binopfun (X Y : setwithbinop) : binopfun X Y -> (X -> Y)
  := @pr1 _ _.
...
Lemma isbinopfuncomp {X Y Z : setwithbinop} (f : binopfun X Y)
      (g : binopfun Y Z) : isbinopfun (funcomp (pr1 f) (pr1 g)).
Proof.
  intros.
...
Definition binopfuncomp {X Y Z : setwithbinop} (f : binopfun X Y)
           (g : binopfun Y Z) : binopfun X Z
  := binopfunpair (funcomp (pr1 f) (pr1 g)) (isbinopfuncomp f g).


Definition binopmono (X Y : setwithbinop) : UU
  := total2 (fun f : incl X Y => isbinopfun (pr1 f)).
Definition binopmonopair {X Y : setwithbinop} (f : incl X Y)
           (is : isbinopfun f) : binopmono X Y := tpair _  f is.
Definition pr1binopmono (X Y : setwithbinop) : binopmono X Y -> incl X Y
  := @pr1 _ _.
...
Definition binopmonocomp {X Y Z : setwithbinop} (f : binopmono X Y)
           (g : binopmono Y Z) : binopmono X Z
  := binopmonopair (inclcomp (pr1 f) (pr1 g)) (isbinopfuncomp f g).

Definition binopiso (X Y : setwithbinop) : UU
  := total2 (fun f : weq X Y => isbinopfun f).
Definition binopisopair {X Y : setwithbinop} (f : weq X Y)
           (is : isbinopfun f) : binopiso X Y := tpair _  f is.
Definition pr1binopiso (X Y : setwithbinop) : binopiso X Y -> weq X Y
  := @pr1 _ _.
...
Definition binopisocomp {X Y Z : setwithbinop} (f : binopiso X Y)
           (g : binopiso Y Z) : binopiso X Z
  := binopisopair (weqcomp (pr1 f) (pr1 g)) (isbinopfuncomp f g).

Lemma isbinopfuninvmap {X Y : setwithbinop} (f : binopiso X Y) :
  isbinopfun (invmap (pr1 f)).
Proof.
...
Definition invbinopiso {X Y : setwithbinop} (f : binopiso X Y) : binopiso Y X
  := binopisopair (invweq (pr1 f)) (isbinopfuninvmap f).
...
Proof.
  ...
  assert (h : homot (funcomp f (fun y0 : Y => op (f x) y0))
                    (funcomp (fun x0 : X => op x x0) f)).
  {
...
Proof.
  ...
  assert (h : homot (funcomp f (fun y0 : Y => op y0 (f x)))
                    (funcomp (fun x0 : X => op x0 x) f)).
  {
...
Proof.
  intros.
  apply (dirprodpair (islcancelablemonob f x (pr1 is))
                     (isrcancelablemonob f x (pr2 is))).
Defined.
...
Proof.
  intros.
  apply (dirprodpair (islinvertibleisob f x (pr1 is))
                     (isrinvertibleisob f x (pr2 is))).
Defined.
...
Proof.
  ...
  - apply (pr2 (pr1 f)).
  - assert (h : homot (funcomp (fun x0 : X => op x x0) f)
                      (fun x0 : X => op (f x) (f x0))).
    {
...
Proof.
  ...
  - apply (pr2 (pr1 f)).
  - assert (h : homot (funcomp (fun x0 : X => op x0 x) f)
                      (fun x0 : X => op (f x0) (f x))).
    {
...
Proof.
  intros.
  apply (dirprodpair (islinvertibleisof f x (pr1 is))
                     (isrinvertibleisof f x (pr2 is))).
Defined.


Lemma isassocmonob {X Y : setwithbinop} (f : binopmono X Y)
      (is : isassoc (@op Y)) : isassoc (@op X).
Proof.
  intros.
...
Lemma iscommmonob {X Y : setwithbinop} (f : binopmono X Y)
      (is : iscomm (@op Y)) : iscomm (@op X).
Proof.
...
Lemma isassocisof {X Y : setwithbinop} (f : binopiso X Y)
      (is : isassoc (@op X)) : isassoc (@op Y).
Proof. intros. apply (isassocmonob (invbinopiso f) is). Defined.

Opaque isassocisof.

Lemma iscommisof {X Y : setwithbinop} (f : binopiso X Y) (is : iscomm (@op X)) :
  iscomm (@op Y).
Proof. intros. apply (iscommmonob (invbinopiso f) is). Defined.

Opaque iscommisof.

Lemma isunitisof {X Y : setwithbinop} (f : binopiso X Y) (unx : X)
      (is : isunit (@op X) unx) : isunit (@op Y) (f unx).
Proof.
...
Definition isunitalisof {X Y : setwithbinop} (f : binopiso X Y)
           (is : isunital (@op X)) : isunital (@op Y)
  := isunitalpair (f (pr1 is)) (isunitisof f (pr1 is) (pr2 is)).

Lemma isunitisob {X Y : setwithbinop} (f : binopiso X Y) (uny : Y)
      (is : isunit (@op Y) uny) : isunit (@op X) ((invmap f) uny).
Proof.
...
Definition isunitalisob {X Y : setwithbinop} (f : binopiso X Y)
           (is : isunital (@op Y)) : isunital (@op X)
  := isunitalpair ((invmap f) (pr1 is)) (isunitisob f (pr1 is) (pr2 is)).


Definition ismonoidopisof {X Y : setwithbinop} (f : binopiso X Y)
           (is : ismonoidop (@op X)) : ismonoidop (@op Y)
  := dirprodpair (isassocisof f (pr1 is)) (isunitalisof f (pr2 is)).

Definition ismonoidopisob {X Y : setwithbinop} (f : binopiso X Y)
           (is : ismonoidop (@op Y)) : ismonoidop (@op X)
  := dirprodpair (isassocisob f (pr1 is)) (isunitalisob f (pr2 is)).

Lemma isinvisof {X Y : setwithbinop} (f : binopiso X Y) (unx : X)
      (invx : X -> X) (is : isinv (@op X) unx invx) :
...
Definition isgropisof {X Y : setwithbinop} (f : binopiso X Y)
           (is : isgrop (@op X)) : isgrop (@op Y)
  :=  tpair _ (ismonoidopisof f is)
            (tpair _ (funcomp (invmap (pr1 f)) (funcomp (grinv_is is) (pr1 f)))
                   (isinvisof f (unel_is is) (grinv_is is) (pr2 (pr2 is)))).

Lemma isinvisob {X Y : setwithbinop} (f : binopiso X Y) (uny : Y)
      (invy : Y -> Y) (is : isinv (@op Y) uny invy) :
  isinv (@op X) (invmap (pr1 f) uny)
        (funcomp (pr1 f) (funcomp invy (invmap (pr1 f)))).
Proof. intros. apply (isinvisof (invbinopiso f) uny invy is). Defined.

Opaque isinvisob.

Definition isgropisob {X Y : setwithbinop} (f : binopiso X Y)
           (is : isgrop (@op Y)) : isgrop (@op X)
  := tpair _ (ismonoidopisob f is)
           (tpair _  (funcomp (pr1 f) (funcomp (grinv_is is) (invmap (pr1 f))))
                  (isinvisob f (unel_is is) (grinv_is is) (pr2 (pr2 is)))).

Definition isabmonoidopisof {X Y : setwithbinop} (f : binopiso X Y)
           (is : isabmonoidop (@op X)) : isabmonoidop (@op Y)
  := tpair _ (ismonoidopisof f is) (iscommisof f (commax_is is)).

Definition isabmonoidopisob {X Y : setwithbinop} (f : binopiso X Y)
           (is : isabmonoidop (@op Y)) : isabmonoidop (@op X)
  := tpair _ (ismonoidopisob f is) (iscommisob f (commax_is is)).


Definition isabgropisof {X Y : setwithbinop} (f : binopiso X Y)
           (is : isabgrop (@op X)) : isabgrop (@op Y)
  := tpair _ (isgropisof f is) (iscommisof f (commax_is is)).

Definition isabgropisob {X Y : setwithbinop} (f : binopiso X Y)
           (is : isabgrop (@op Y)) : isabgrop (@op X)
  := tpair _ (isgropisob f is) (iscommisob f (commax_is is)).
...
Definition subsetswithbinoppair {X : setwithbinop} :
  Π (t : hsubtypes X),
  (λ A : hsubtypes X, issubsetwithbinop op A) t
...
Definition subsetswithbinopconstr {X : setwithbinop} :
  ...
  (λ A : hsubtypes X, issubsetwithbinop op A) t
  → Σ A : hsubtypes X, issubsetwithbinop op A
  := @subsetswithbinoppair X.
Definition pr1subsetswithbinop (X : setwithbinop) :
  @subsetswithbinop X -> hsubtypes X
...
Definition carrierofasubsetwithbinop {X : setwithbinop}
           (A : @subsetswithbinop X) : setwithbinop.
Proof.
  intros.
  set (aset := (hSetpair
                  (carrier A)
                  (isasetsubset (pr1carrier A) (setproperty X)
                                (isinclpr1carrier A))) : hSet).
  split with aset.
  set (subopp := (fun a a' : A => carrierpair A (op (pr1carrier _ a)
                                                 (pr1carrier _ a'))
                                           (pr2 A a a')) : (A -> A -> A)).
...
Lemma iscompbinoptransrel {X : setwithbinop} (R : hrel X) (ist : istrans R)
      (isb : isbinophrel R) : iscomprelrelfun2 R R (@op X).
Proof.
...
Definition binophrel {X : setwithbinop} : UU
  := total2 (fun R : hrel X => isbinophrel R).
Definition binophrelpair {X : setwithbinop} :
  Π (t : hrel X),
  (λ R : hrel X, isbinophrel R) t → Σ R : hrel X, isbinophrel R
  := tpair (fun R : hrel X => isbinophrel R).
Definition pr1binophrel (X : setwithbinop) :
  @binophrel X -> hrel X := @pr1 _ (fun R : hrel X => isbinophrel R).
Coercion pr1binophrel : binophrel >-> hrel.

Definition binoppo {X : setwithbinop} : UU
  := total2 (fun R : po X => isbinophrel R).
...
Definition pr1binoppo (X : setwithbinop) : @binoppo X -> po X
  := @pr1 _ (fun R : po X => isbinophrel R).
Coercion pr1binoppo : binoppo >-> po.

Definition binopeqrel {X : setwithbinop} : UU
  := total2 (fun R : eqrel X => isbinophrel R).
Definition binopeqrelpair {X : setwithbinop} :
  Π (t : eqrel X),
  (λ R : eqrel X, isbinophrel R) t → Σ R : eqrel X, isbinophrel R
...
Definition setwithbinopquot {X : setwithbinop} (R : @binopeqrel X) :
  setwithbinop.
Proof.
  intros. split with (setquotinset R).
  set (qt  := setquot R). set (qtset := setquotinset R).
  assert (iscomp : iscomprelrelfun2 R R op)
    by apply (iscompbinoptransrel R (eqreltrans R) (pr2 R)).
  set (qtmlt := setquotfun2 R R op iscomp).
  simpl. unfold binop. apply qtmlt.
Defined.


Definition ispartbinophrel {X : setwithbinop} (S : hsubtypes X) (R : hrel X) :
  UU := dirprod (Π a b c : X, S c -> R a b -> R (op c a) (op c b))
                (Π a b c : X, S c -> R a b -> R (op a c) (op b c)).

Definition isbinoptoispartbinop {X : setwithbinop} (S : hsubtypes X)
           (L : hrel X) (is : isbinophrel L) : ispartbinophrel S L.
Proof.
  intros X S L.
...
Definition ispartbinophrellogeqf {X : setwithbinop} (S : hsubtypes X)
           {L R : hrel X} (lg : hrellogeq L R) (isl : ispartbinophrel S L) :
  ispartbinophrel S R.
Proof.
  intros. split.
  - intros a b c is rab.
...
Lemma ispartbinophrelif {X : setwithbinop} (S : hsubtypes X) (R : hrel X)
      (is : iscomm (@op X))
      (isl : Π a b c : X, S c -> R a b -> R (op c a) (op c b)) :
  ispartbinophrel S R.
Proof.
...
Definition isinvbinophrel {X : setwithbinop} (R : hrel X) : UU
  := dirprod (Π a b c : X, R (op c a) (op c b) ->  R a b)
             (Π a b c : X, R (op a c) (op b c) -> R a b).

Definition isinvbinophrellogeqf {X : setwithbinop} {L R : hrel X}
           (lg : hrellogeq L R) (isl : isinvbinophrel L) : isinvbinophrel R.
Proof.
  intros. split.
  - intros a b c rab.
    apply ((pr1 (lg _ _) ((pr1 isl) _ _ _ (pr2 (lg  _ _) rab)))).
...
Lemma isapropisinvbinophrel {X : setwithbinop} (R : hrel X) :
  isaprop (isinvbinophrel R).
Proof.
  intros. apply isapropdirprod.
  - apply impred. intro a.
    apply impred. intro b.
...
Definition ispartinvbinophrel {X : setwithbinop} (S : hsubtypes X)
           (R : hrel X) : UU
  := dirprod (Π a b c : X, S c -> R (op c a) (op c b) -> R a b)
             (Π a b c : X, S c -> R (op a c) (op b c) -> R a b).

Definition isinvbinoptoispartinvbinop {X : setwithbinop} (S : hsubtypes X)
           (L : hrel X) (is : isinvbinophrel L) : ispartinvbinophrel S L.
Proof.
  intros X S L.
...
Definition ispartinvbinophrellogeqf {X : setwithbinop} (S : hsubtypes X)
           {L R : hrel X} (lg : hrellogeq L R) (isl : ispartinvbinophrel S L) :
  ispartinvbinophrel S R.
Proof.
  intros. split.
  - intros a b c s rab.
...
Lemma ispartinvbinophrelif {X : setwithbinop} (S : hsubtypes X) (R : hrel X)
      (is : iscomm (@op X))
      (isl : Π a b c : X, S c -> R (op c a) (op c b) -> R a b) :
  ispartinvbinophrel S R.
Proof.
...
Lemma binophrelandfun {X Y : setwithbinop} (f : binopfun X Y) (R : hrel Y)
      (is : @isbinophrel Y R) : @isbinophrel X (fun x x' => R (f x) (f x')).
Proof.
  intros.
...
Lemma ispartbinophrelandfun {X Y : setwithbinop} (f : binopfun X Y)
      (SX : hsubtypes X) (SY : hsubtypes Y)
      (iss : Π x : X, (SX x) -> (SY (f x))) (R : hrel Y)
      (is : @ispartbinophrel Y SY R) :
  @ispartbinophrel X SX (fun x x' => R (f x) (f x')).
Proof.
  intros.
  set (ish := (pr2 f) : Π a0 b0, paths (f (op a0 b0)) (op (f a0) (f b0))).
  split.
...
Lemma ispartinvbinophrelandfun {X Y : setwithbinop} (f : binopfun X Y)
      (SX : hsubtypes X) (SY : hsubtypes Y)
      (iss : Π x : X, (SX x) -> (SY (f x))) (R : hrel Y)
      (is : @ispartinvbinophrel Y SY R) :
  @ispartinvbinophrel X SX (fun x x' => R (f x) (f x')).
Proof.
  intros.
  set (ish := (pr2 f) : Π a0 b0, paths (f (op a0 b0)) (op (f a0) (f b0))).
  split.
...
Lemma isbinopquotrel {X : setwithbinop} (R : @binopeqrel X) {L : hrel X}
      (is : iscomprelrel R L) (isl : isbinophrel L) :
  @isbinophrel (setwithbinopquot R) (quotrel is).
Proof.
...
Proof.
  ...
  apply (fun xy xy' : dirprod X Y =>
           dirprodpair (op (pr1 xy) (pr1 xy')) (op (pr2 xy) (pr2 xy'))).
Defined.
...
Definition setwith2binop : UU
  := total2 (fun X : hSet => dirprod (binop X) (binop X)).
...
Definition setwithbinop1 (X : setwith2binop) : setwithbinop
  := setwithbinoppair (pr1 X) (@op1 X).
Definition setwithbinop2 (X : setwith2binop) : setwithbinop
  := setwithbinoppair (pr1 X) (@op2 X).
...
Lemma isapropistwobinopfun {X Y : setwith2binop} (f : X -> Y) :
  isaprop (istwobinopfun f).
Proof.
...
Definition twobinopfun (X Y : setwith2binop) : UU
  := total2 (fun f : X -> Y => istwobinopfun f).
Definition twobinopfunpair {X Y : setwith2binop} (f : X -> Y)
           (is : istwobinopfun f) : twobinopfun X Y := tpair _ f is.
Definition pr1twobinopfun (X Y : setwith2binop) : twobinopfun X Y -> (X -> Y)
  := @pr1 _ _.
...
Proof.
  ...
  change (isofhlevel 2 (X -> Y)).
  apply impred. intro. apply (setproperty Y). refine (isinclpr1 _ _).
  intro. apply isapropistwobinopfun.
Defined.


Lemma istwobinopfuncomp {X Y Z : setwith2binop} (f : twobinopfun X Y)
      (g : twobinopfun Y Z) : istwobinopfun (funcomp (pr1 f) (pr1 g)).
Proof.
  intros.
...
Definition twobinopfuncomp {X Y Z : setwith2binop} (f : twobinopfun X Y)
           (g : twobinopfun Y Z) : twobinopfun X Z
  := twobinopfunpair (funcomp (pr1 f) (pr1 g)) (istwobinopfuncomp f g).


Definition twobinopmono (X Y : setwith2binop) : UU
  := total2 (fun f : incl X Y => istwobinopfun f).
Definition twobinopmonopair {X Y : setwith2binop} (f : incl X Y)
           (is : istwobinopfun f) : twobinopmono X Y := tpair _  f is.
Definition pr1twobinopmono (X Y : setwith2binop) :
  twobinopmono X Y -> incl X Y := @pr1 _ _.
Coercion pr1twobinopmono : twobinopmono >-> incl.

Definition twobinopincltotwobinopfun (X Y : setwith2binop) :
  twobinopmono X Y -> twobinopfun X Y
  := fun f => twobinopfunpair (pr1 (pr1 f)) (pr2 f).
...
Definition twobinopmonocomp {X Y Z : setwith2binop} (f : twobinopmono X Y)
           (g : twobinopmono Y Z) : twobinopmono X Z
  := twobinopmonopair (inclcomp (pr1 f) (pr1 g)) (istwobinopfuncomp f g).

Definition twobinopiso (X Y : setwith2binop) : UU
  := total2 (fun f : weq X Y => istwobinopfun f).
Definition twobinopisopair {X Y : setwith2binop} (f : weq X Y)
           (is : istwobinopfun f) : twobinopiso X Y := tpair _  f is.
Definition pr1twobinopiso (X Y : setwith2binop) :
  twobinopiso X Y -> weq X Y := @pr1 _ _.
Coercion pr1twobinopiso : twobinopiso >-> weq.

Definition twobinopisototwobinopmono (X Y : setwith2binop) :
  twobinopiso X Y -> twobinopmono X Y
  := fun f => twobinopmonopair (pr1 f) (pr2 f).
...
Definition twobinopisocomp {X Y Z : setwith2binop} (f : twobinopiso X Y)
           (g : twobinopiso Y Z) : twobinopiso X Z
  := twobinopisopair (weqcomp (pr1 f) (pr1 g)) (istwobinopfuncomp f g).
...
Lemma isldistrmonob {X Y : setwith2binop} (f : twobinopmono X Y)
      (is : isldistr (@op1 Y) (@op2 Y)) : isldistr (@op1 X) (@op2 X).
Proof.
  intros.
  set (ax1f := pr1 (pr2 f)). set (ax2f := pr2 (pr2 f)).
  intros a b c. apply (invmaponpathsincl _ (pr2 (pr1 f))).
  change (paths ((pr1 f) (op2 c (op1 a b)))
                ((pr1 f) (op1 (op2 c a) (op2 c b)))).
...
Proof.
  ...
  intros a b c.
  apply (invmaponpathsincl _ (pr2 (pr1 f))).
  change (paths ((pr1 f) (op2 (op1 a b) c))
                ((pr1 f) (op1 (op2 a c) (op2 b c)))).
...
Definition isdistrmonob {X Y : setwith2binop} (f : twobinopmono X Y)
           (is : isdistr (@op1 Y) (@op2 Y)) : isdistr (@op1 X) (@op2 X)
  := dirprodpair (isldistrmonob f (pr1 is)) (isrdistrmonob f (pr2 is)).

Notation isldistrisob := isldistrmonob.
Notation isrdistrisob := isrdistrmonob.
Notation isdistrisob := isdistrmonob.

Lemma isldistrisof  {X Y : setwith2binop} (f : twobinopiso X Y)
      (is : isldistr (@op1 X) (@op2 X)) : isldistr (@op1 Y) (@op2 Y).
Proof. intros. apply (isldistrisob (invtwobinopiso f) is). Defined.

Lemma isrdistrisof {X Y : setwith2binop} (f : twobinopiso X Y)
      (is : isrdistr (@op1 X) (@op2 X)) : isrdistr (@op1 Y) (@op2 Y).
Proof. intros. apply (isrdistrisob (invtwobinopiso f) is). Defined.

Lemma isdistrisof  {X Y : setwith2binop} (f : twobinopiso X Y)
      (is : isdistr (@op1 X) (@op2 X)) : isdistr (@op1 Y) (@op2 Y).
...
Definition iscommrngopsisof {X Y : setwith2binop} (f : twobinopiso X Y)
           (is : iscommrngops (@op1 X) (@op2 X)) :
  iscommrngops (@op1 Y) (@op2 Y)
...
Definition iscommrngopsisob {X Y : setwith2binop} (f : twobinopiso X Y)
           (is : iscommrngops (@op1 Y) (@op2 Y)) :
  iscommrngops (@op1 X) (@op2 X)
...
Definition issubsetwith2binop {X : setwith2binop} (A : hsubtypes X) : UU
  := dirprod (Π a a' : A, A (op1 (pr1 a) (pr1 a')))
             (Π a a' : A, A (op2 (pr1 a) (pr1 a'))).
...
Definition subsetswith2binoppair {X : setwith2binop} :
  Π (t : hsubtypes X),
  (λ A : hsubtypes X, issubsetwith2binop A) t
...
Definition pr1subsetswith2binop (X : setwith2binop) :
  @subsetswith2binop X -> hsubtypes X
...
Definition carrierofsubsetwith2binop {X : setwith2binop}
           (A : @subsetswith2binop X) : setwith2binop.
Proof.
  intros.
  set (aset := (hSetpair (carrier A)
                         (isasetsubset (pr1carrier A) (setproperty X)
                                       (isinclpr1carrier A))) : hSet).
  split with aset.
  set (subopp1 := (fun a a' : A => carrierpair
                                  A (op1 (pr1carrier _ a) (pr1carrier _ a'))
                                  (pr1 (pr2 A) a a')) : (A -> A -> A)).
  set (subopp2 := (fun a a' : A => carrierpair
                                  A (op2 (pr1carrier _ a) (pr1carrier _ a'))
                                  (pr2 (pr2 A) a a')) : (A -> A -> A)).
...
Definition is2binophrel {X : setwith2binop} (R : hrel X) : UU
  := dirprod (@isbinophrel (setwithbinop1 X) R)
             (@isbinophrel (setwithbinop2 X) R).

Lemma isapropis2binophrel {X : setwith2binop} (R : hrel X) :
  isaprop (is2binophrel R).
Proof.
  intros. apply (isofhleveldirprod 1).
...
Lemma iscomp2binoptransrel {X : setwith2binop} (R : hrel X) (is : istrans R)
      (isb : is2binophrel R) :
  dirprod (iscomprelrelfun2 R R (@op1 X))
          (iscomprelrelfun2 R R (@op2 X)).
Proof.
  intros. split.
...
Definition twobinophrel {X : setwith2binop} : UU
  := total2 (fun R : hrel X => is2binophrel R).
Definition twobinophrelpair {X : setwith2binop} :
  Π (t : hrel X),
  (λ R : hrel X, is2binophrel R) t → Σ R : hrel X, is2binophrel R
...
Definition twobinoppo {X : setwith2binop} : UU
  := total2 (fun R : po X => is2binophrel R).
Definition twobinoppopair {X : setwith2binop} :
  Π (t : po X),
  (λ R : po X, is2binophrel R) t → Σ R : po X, is2binophrel R
  := tpair (fun R : po X => is2binophrel R).
Definition pr1twobinoppo (X : setwith2binop) :
  @twobinoppo X -> po X := @pr1 _ (fun R : po X => is2binophrel R).
Coercion pr1twobinoppo : twobinoppo >-> po.

Definition twobinopeqrel {X : setwith2binop} : UU
  := total2 (fun R : eqrel X => is2binophrel R).
Definition twobinopeqrelpair {X : setwith2binop} :
  Π (t : eqrel X),
  (λ R : eqrel X, is2binophrel R) t → Σ R : eqrel X, is2binophrel R
...
Definition setwith2binopquot {X : setwith2binop} (R : @twobinopeqrel X) :
  setwith2binop.
Proof.
  intros. split with (setquotinset R).
  set (qt := setquot R). set (qtset := setquotinset R).
...
Proof.
  ...
  apply (dirprodpair (fun xy xy' : dirprod X Y =>
                        dirprodpair (op1 (pr1 xy) (pr1 xy'))
                                    (op1 (pr2 xy) (pr2 xy')))
                     (fun xy xy' :  dirprod X Y  =>
                        dirprodpair (op2 (pr1 xy) (pr1 xy'))
                                    (op2 (pr2 xy) (pr2 xy')))).
Defined.
