...
Lemma islcancelableif {X : hSet} (opp : binop X) (x : X)
      (is : Π a b : X, paths (opp x a) (opp x b) -> paths a b) :
  islcancelable opp x.
Proof.
...
Lemma isrcancelableif {X : hSet} (opp : binop X) (x : X)
      (is : Π a b : X, paths (opp a x) (opp b x) -> paths a b) :
  isrcancelable opp x.
Proof.
  intros. apply isinclbetweensets.
  - apply (setproperty X).
...
Definition iscancelableif {X : hSet} (opp : binop X) (x : X)
  ...
  iscancelable opp x
  := dirprodpair (islcancelableif opp x isl) (isrcancelableif opp x isr).
...
Definition linvpair (X : monoid) (x : X) : UU
  := total2 (fun x' : X => paths (x' * x) 1).
Definition pr1linvpair (X : monoid) (x : X) : linvpair X x -> X := @pr1 _ _.

Definition linvpairxy (X : monoid) (x y : X)
           (x' : linvpair X x) (y' : linvpair X y) : linvpair X (x * y).
Proof.
...
Definition lcanfromlinv (X : monoid) (a b c : X) (c' : linvpair X c)
           (e : paths (c * a) (c * b)) : paths a b.
Proof.
  intros.
...
Definition rinvpair (X : monoid) (x : X) : UU
  := total2 (fun x' : X => paths (x * x') 1).
Definition pr1rinvpair (X : monoid) (x : X) : rinvpair X x -> X := @pr1 _ _.

Definition rinvpairxy (X : monoid) (x y : X)
           (x' : rinvpair X x) (y' : rinvpair X y) : rinvpair X (x * y).
Proof.
  intros. split with ((pr1 y') * (pr1 x')).
...
Definition rcanfromrinv (X : monoid) (a b c : X) (c' : rinvpair X c)
           (e : paths (a * c ) (b * c)) : paths a b.
Proof.
  intros.
...
Lemma pathslinvtorinv (X : monoid) (x : X) (x' : linvpair X x)
      (x'' : rinvpair X x) : paths (pr1 x') (pr1 x'').
Proof.
...
Proof.
  ...
  apply (invmaponpathsincl
           _ (isinclpr1 _ (fun a => isapropdirprod _ _ (setproperty X _ _)
                                                (setproperty X _ _)))).
  apply (pathslinvtorinv X x (invtolinv X x x') (invtorinv X x x'')).
Defined.

Definition invpairxy (X : monoid) (x y : X)
           (x' : invpair X x) (y' : invpair X y) : invpair X (x * y).
Proof.
...
Lemma grfrompathsxy (X : gr) {a b : X} (e : paths a b) :
  paths (op a (grinv X b)) (unel X).
Proof.
  intros.
...
Lemma grtopathsxy (X : gr) {a b : X} (e : paths (op a (grinv X b)) (unel X) ) :
  paths a b .
Proof.
  intros.
...
Definition rigneq0andmultlinv (X : rig) (n m : X)
           (isnm : neg (paths (n * m) 0)%rig) : neg (paths n 0)%rig.
Proof.
...
Definition rigneq0andmultrinv (X : rig) (n m : X)
           (isnm : neg (paths (n * m) 0)%rig) : neg (paths m 0)%rig.
Proof.
...
Definition rngneq0andmultlinv (X : rng) (n m : X)
           (isnm : neg (paths (n * m) 0)) : neg (paths n 0).
Proof.
  intros. intro e. rewrite e in isnm.
...
Definition rngneq0andmultrinv (X : rng) (n m : X)
           (isnm : neg (paths (n * m) 0)) : neg (paths m 0).
Proof.
  intros. intro e. rewrite e in isnm.
...
Definition rngpossubmonoid (X : rng) {R : hrel X} (is1 : isrngmultgt X R)
           (is2 : R 1 0) : @submonoids (rngmultmonoid X).
Proof.
...
Lemma isinvrngmultgtif (X : rng) {R : hrel X} (is0 : @isbinophrel X R)
      (is1 : isrngmultgt X R) (nc : neqchoice R) (isa : isasymm R) :
  isinvrngmultgt X R.
Proof.
  intros. split.
  - intros a b rab0 ra0.
    ...
    destruct (nc _ _ int) as [ g | l ]. apply g.
...
Some of the notions considered in this section were introduced in C. Mulvey
"Intuitionistic algebra and representations of rings". See also P.T. Johnstone
"Rings, fields and spectra". We only consider here the strongest ("geometric")
forms of the conditions of integrality and of being a field. In particular all
our fileds have decidable equality and p-adic numbers or reals are not fileds
in the sense of the definitions considered here.   *)
...
Lemma isnonzerolinvel (X : rng) (is : isnonzerorng X) (x : X)
      (x' : multlinvpair X x) : neg (paths (pr1 x') 0).
Proof.
  intros.
...
Lemma isnonzerorinvel (X : rng) (is : isnonzerorng X) (x : X)
      (x' : multrinvpair X x) : neg (paths (pr1 x') 0).
Proof.
  intros.
...
Lemma isnonzerofromlinvel (X : rng) (is : isnonzerorng X) (x : X)
      (x' : multlinvpair X x) : neg (paths x 0).
Proof.
...
Lemma isnonzerofromrinvel (X : rng) (is : isnonzerorng X) (x : X)
      (x' : multrinvpair X x) : neg (paths x 0).
Proof.
...
Definition isintdom (X : commrng) : UU
  := dirprod (isnonzerorng X)
             (Π (a1 a2 : X),
              paths (a1 * a2) 0 -> hdisj (eqset a1 0) (eqset a2 0)).
...
Definition intdomax (X : intdom) :
  Π (a1 a2 : X),
  paths (a1 * a2) 0 -> hdisj (eqset a1 0) (eqset a2 0) := pr2 (pr2 X).

(** **** Computational lemmas for integral domains *)

Lemma intdomax2l (X : intdom) (x y : X) (is : paths (x * y) 0)
      (ne : neg (paths x 0)) : paths y 0.
Proof.
  intros.
...
Lemma intdomax2r (X : intdom) (x y : X) (is : paths (x * y) 0)
      (ne : neg (paths y 0)) : paths x 0.
Proof.
  intros.
  set (int := intdomax X _ _ is). generalize ne.
...
Lemma intdomlcan (X : intdom) :
  Π (a b c : X), neg (paths c 0) -> paths (c * a) (c * b) -> paths a b.
Proof.
  intros X a b c ne e.
  ...
  generalize int. simpl. apply (@hinhuniv _ (hProppair _ int')).
  intro ene. destruct ene as [ e'' | ne' ].
  destruct (ne e''). intro. apply ne'.
Defined.

Opaque intdomlcan.

Lemma intdomrcan (X : intdom) :
  Π (a b c : X), neg (paths c 0) -> paths (a * c) (b * c) -> paths a b.
Proof.
  ...
  generalize int. simpl. apply (@hinhuniv _ (hProppair _ int')).
  intro ene. destruct ene as [ e'' | ne' ]. intro. apply e''.
  destruct (ne ne').
Defined.
...
Lemma intdomiscancelable (X : intdom) (x : X) (is : neg (paths x 0)) :
  iscancelable (@op2 X) x.
Proof.
...
Definition intdomnonzerosubmonoid (X : intdom) :
...
Proof.
  ...
  apply (tpair _ (pr1 x) g). split with (- (pr1 x)). simpl.
  apply rngtogt0. apply is0. rewrite (rngminusminus X _).
  apply l.
Defined.
...
Definition fldchoice {X : fld} (x : X) : coprod (multinvpair X x) (paths x 0)
  := pr2 (pr2 X) x.

Definition fldmultinvpair (X : fld) (x : X) (ne : neg (paths x 0)) :
  multinvpair X x.
Proof.
  intros. destruct (fldchoice x) as [ ne0 | e0 ].
  apply ne0. destruct (ne e0).
Defined.

Definition fldmultinv {X : fld} (x : X) (ne : neg (paths x 0)) : X
  := pr1 (fldmultinvpair X x ne).


(** **** Field of fractions of an integral domain with decidable equality *)

Definition fldfracmultinvint (X : intdom) (is : isdeceq X)
           (xa : dirprod X (intdomnonzerosubmonoid X)) :
...
Proof.
  ...
  apply (dirprodpair
           (pr1 (pr2 xa)) (tpair (fun x => neg (paths x 0)) (pr1 xa) ne0)).
Defined.

(** Note: we choose a strange from the mathematicians perspective approach to
the definition of the multiplicative inverse on non-zero elements of a field due
to the current, somewhat less than satisfactory, situation with computational
behavior of our construction of set-quotients. The particular problem is that
the weak equivalence between "quotient of subtype" and "subtype of a quotient"
is not isomorphism in the syntactic category. This can be corrected by
extension of the type system with tfc-terms. See discussion in hSet.v *)
...
Definition fldfracmultinv0 (X : intdom) (is : isdeceq X)
  ...
  commrngfrac X (intdomnonzerosubmonoid X)
  := setquotfun _ _ _ (fldfracmultinvintcomp X is) x.


Lemma nonzeroincommrngfrac (X : commrng) (S : @submonoids (rngmultmonoid X))
      (xa : dirprod X S)
      (ne : neg (paths (setquotpr (eqrelcommrngfrac X S) xa)
                       (setquotpr _ (dirprodpair 0 (unel S))))) :
  neg (paths (pr1 xa) 0).
Proof.
  ...
  rewrite (rngmult0x X _). rewrite (rngmult0x X _). rewrite (rngmult0x X _).
  rewrite (rngmult0x X _).
  apply idpath.
Defined.
...
Lemma islinvinfldfrac (X : intdom) (is : isdeceq X)
      (x : commrngfrac X (intdomnonzerosubmonoid X)) (ne : neg (paths x 0)) :
  paths ((fldfracmultinv0 X is x) * x) 1.
Proof.
  intros X is.
  assert (int : Π x0, isaprop (neg (paths x0 0) ->
                               paths ((fldfracmultinv0 X is x0) * x0) 1)).
  {
    intro x0.
    apply impred. intro.
    apply (setproperty (commrngfrac X (intdomnonzerosubmonoid X))
                       (fldfracmultinv0 X is x0 * x0) _).
  }
  apply (setquotunivprop _ (fun x0 => hProppair _ (int x0))).
  simpl. intros xa ne.
  change (paths (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                           (dirprodpair ((pr1 (fldfracmultinvint X is xa)) *
                                         (pr1 xa))
                                        (@op (intdomnonzerosubmonoid X)
                                             (pr2 (fldfracmultinvint X is xa))
                                             (pr2 xa))))
  ...
  - apply hinhpr.
    ...
    rewrite (rngcomm2 X a x). rewrite (rngrunax2 X _). rewrite (rngrunax2 X _).
    rewrite (rngrunax2 X _). rewrite (rnglunax2 X _).
    apply idpath.
Defined.

Opaque islinvinfldfrac.

Lemma isrinvinfldfrac (X : intdom) (is : isdeceq X)
      (x : commrngfrac X (intdomnonzerosubmonoid X)) (ne : neg (paths x 0)) :
  paths (x * (fldfracmultinv0 X is x)) 1.
Proof.
...
Definition tofldfrac (X : intdom) (is : isdeceq X) (x : X) : fldfrac X is
  := setquotpr
       _ (dirprodpair x (tpair (fun x => neg (paths x 0)) 1 (nonzeroax X))).
...
Lemma isunital1funtofldfrac (X : intdom) (is : isdeceq X) :
  paths (tofldfrac X is 0) 0.
...
Definition isbinop2funtofldfrac (X : intdom) (is : isdeceq X) :
  @isbinopfun (rngmultmonoid X) (rngmultmonoid (fldfrac X is))
              (tofldfrac X is)
  := isbinopfuntoabmonoidfrac (rngmultabmonoid X) (intdomnonzerosubmonoid X).

Opaque isbinop2funtofldfrac.

Lemma isunital2funtofldfrac (X : intdom) (is : isdeceq X) :
  paths (tofldfrac X is 1) 1.
Proof. intros. apply idpath. Defined.

Opaque isunital2funtofldfrac.

Definition ismultmonoidfuntofldfrac (X : intdom) (is : isdeceq X) :
  @ismonoidfun (rngmultmonoid X) (rngmultmonoid (fldfrac X is))
               (tofldfrac X is)
...
Definition isincltofldfrac (X : intdom) (is : isdeceq X) :
  isincl (tofldfrac X is)
...
Our approach here is slightly different from the tranditional one used for
example in Bourbaki Algebra II, Ch. VI, Section 2 where one starts with a
total ordering on a ring and extends it to its field of fractions. This
situation woud be exemplified by the extension of "greater or equal" from
integers to rationals. We have chosen to use instead as our archetypical example
the extension of "greater" from integers to rationals. There is no particular
difference between the two choices for types with decidable equality but in the
setting of general rings in constructive mathematics the relations such as
"greater" appear to be more fundamental than relations such as
"greater or equal". For example, "greater or equal" on constructive real numbers
can be obtained from "greater" but not vice versa. *)
...
Definition weqfldfracgtint_f (X : intdom) {R : hrel X} (is0 : @isbinophrel X R)
           (is1 : isrngmultgt X R) (is2 : R 1 0) (nc : neqchoice R)
           (xa : dirprod X (intdomnonzerosubmonoid X)) :
  dirprod X (rngpossubmonoid X is1 is2).
Proof.
...
Definition weqfldfracgt_f (X : intdom) (is : isdeceq X) {R : hrel X}
...
Definition weqfldfracgtint_b (X : intdom) {R : hrel X} (is1 : isrngmultgt X R)
           (is2 : R 1 0) (ir : isirrefl R)
           (xa : dirprod X (rngpossubmonoid X is1 is2)) :
  dirprod X (intdomnonzerosubmonoid X)
  := dirprodpair (pr1 xa) (tpair _ (pr1 (pr2 xa)) (rtoneq ir (pr2 (pr2 xa)))).


Lemma weqfldfracgtintcomp_b (X : intdom) {R : hrel X} (is1 : isrngmultgt X R)
      (is2 : R 1 0) (ir : isirrefl R) :
...
Definition weqfldfracgt_b (X : intdom) (is : isdeceq X) {R : hrel X}
...
Definition weqfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
           (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
           (nc : neqchoice R) (ir : isirrefl R) :
...
Proof.
  ...
  assert (egf : Π a, paths (g (f a)) a).
  {
    ...
    change (paths (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                             (weqfldfracgtint_b
                                X is1 is2 ir
                                (weqfldfracgtint_f X is0 is1 is2 nc xa)))
  ...
  assert (efg : Π a, paths (f (g a)) a).
  {
    ...
    change (paths (setquotpr _ (weqfldfracgtint_f
                                  X is0 is1 is2 nc
                                  (weqfldfracgtint_b X is1 is2 ir xa)))
                  (setquotpr (eqrelcommrngfrac
                                X (rngpossubmonoid X is1 is2)) xa)).
    apply weqpathsinsetquot. simpl. apply hinhpr.
...
Lemma isrngfunweqfldfracgt_b (X : intdom) (is : isdeceq X) {R : hrel X}
      (is1 : isrngmultgt X R) (is2 : R 1 0) (ir : isirrefl R) :
  isrngfun (weqfldfracgt_b X is is1 is2 ir).
Proof.
  intros.
  set (g :=  weqfldfracgt_b X is is1 is2 ir).
    ...
    + unfold isbinopfun.
      ...
      apply (setquotuniv2prop
               _ (fun x x' : commrngfrac X (rngpossubmonoid X is1 is2) =>
                    hProppair _ (setproperty (fldfrac X is)
                                             (g (x + x')) ((g x) + (g x'))))).
      intros xa1 xa2.
      change (paths (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                               (g0 (commrngfracop1int
                                      X (rngpossubmonoid X is1 is2) xa1 xa2)))
                    (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                               (commrngfracop1int  X (intdomnonzerosubmonoid X)
                                                   (g0 xa1) (g0 xa2)))).
      ...
      apply (invmaponpathsincl
               ...
               (carrierpair
                  (fun x : pr1 X => hProppair (paths x 0 -> empty)
                                           (isapropneg (paths x 0)))
                  (a1 * a2)
                  (fun e : paths (a1 * a2) 0 =>
    ...
    + unfold isbinopfun.
      ...
      apply (setquotuniv2prop
               _ (fun x x' : commrngfrac X (rngpossubmonoid X is1 is2) =>
                    hProppair _ (setproperty (fldfrac X is)
                                             (g (x * x')) ((g x) * (g x'))))).
      intros xa1 xa2.
      change (paths (setquotpr
                       (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                       (g0 (commrngfracop2int X (rngpossubmonoid X is1 is2)
                                              xa1 xa2)))
                    (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
                               (commrngfracop2int X (intdomnonzerosubmonoid X)
                                                  (g0 xa1) (g0 xa2)))).
      apply (maponpaths (setquotpr _)).
      ...
      * destruct xa1 as [ x1 aa1 ]. destruct xa2 as [ x2 aa2 ]. simpl.
        ...
        apply (invmaponpathsincl
                 ...
                 (isinclpr1 _ (fun a => (isapropneg (paths a 0))))
                 (tpair _ (a1 * a2) (rtoneq ir (is1 a1 a2 ia1 ia2)))
                 (carrierpair (fun x : pr1 X => hProppair (paths x 0 -> empty)
                                                       (isapropneg (paths x 0)))
                              (a1 * a2)
                              (fun e : paths (a1 * a2) 0 =>
                                 toneghdisj (dirprodpair (rtoneq ir ia1)
                                                         (rtoneq ir ia2))
                                            (intdomax X a1 a2 e))) (idpath _)).
    + change (paths (setquotpr (eqrelcommrngfrac X (intdomnonzerosubmonoid X))
      ...
      * apply (invmaponpathsincl
                 (@pr1 _ _)
                 (isinclpr1 _ (fun a => (isapropneg (paths a 0))))
                 (tpair _ 1 (rtoneq ir is2))
                 (tpair _ 1 (nonzeroax X))).
        simpl. apply idpath.
Defined.
...
Lemma isrngfunweqfldfracgt_f (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (ir : isirrefl R) :
...
Proof.
  ...
  change (@isrngfun (fldfrac X is) (commrngfrac X (rngpossubmonoid X is1 is2))
                    (invmap int)).
...
Definition fldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
           (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
           (nc : neqchoice R) : hrel (fldfrac X is)
  := fun a b => commrngfracgt
               X (rngpossubmonoid X is1 is2) is0 is1 (fun c r => r)
               (weqfldfracgt_f X is is0 is1 is2 nc a)
               (weqfldfracgt_f X is is0 is1 is2 nc b).

Lemma isrngmultfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (ir : isirrefl R) :
  isrngmultgt (fldfrac X is) (fldfracgt X is is0 is1 is2 nc).
Proof.
  intros.
  refine (rngmultgtandfun
            (rngfunconstr (isrngfunweqfldfracgt_f X is is0 is1 is2 nc ir)) _ _).
  apply isrngmultcommrngfracgt.
Defined.
Opaque isrngmultfldfracgt.

Lemma isrngaddfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (ir : isirrefl R) :
  @isbinophrel (fldfrac X is) (fldfracgt X is is0 is1 is2 nc).
Proof.
  intros.
  refine (rngaddhrelandfun
            (rngfunconstr (isrngfunweqfldfracgt_f X is is0 is1 is2 nc ir)) _ _).
...
Lemma istransfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (isr : istrans R) :
...
Lemma isirreflfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (isr : isirrefl R) :
...
Lemma isasymmfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (isr : isasymm R) :
...
Lemma iscotransfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (isr : iscotrans R) :
...
Lemma isantisymmnegfldfracgt  (X : intdom) (is : isdeceq X) {R : hrel X}
      (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
      (nc : neqchoice R) (ir : isirrefl R) (isr : isantisymmneg R) :
  isantisymmneg (fldfracgt X is is0 is1 is2 nc).
Proof.
  intros.
  assert (int : isantisymmneg
                  (commrngfracgt
                     X (rngpossubmonoid X is1 is2) is0 is1 (fun c r => r))).
  {
    ...
    apply (isantisymmnegabmonoidfracrel
             ...
             (ispartbinopcommrngfracgt
                X (rngpossubmonoid X is1 is2) is0 is1
...
Definition isdecfldfracgt (X : intdom) (is : isdeceq X) {R : hrel X}
           (is0 : @isbinophrel X R) (is1 : isrngmultgt X R) (is2 : R 1 0)
           (nc : neqchoice R) (isa : isasymm R) (isr : isdecrel R) :
  isdecrel (fldfracgt X is is0 is1 is2 nc).
Proof.
  intros. unfold fldfracgt. intros a b. apply isdecabmonoidfracrel.
  apply (pr1 (isinvrngmultgtaspartinvbinophrel X R is0)).
  apply isinvrngmultgtif. apply is0. apply is1. apply nc. apply isa.
  apply isr.
Defined.
...
Definition iscomptofldfrac (X : intdom) (is : isdeceq X) {L : hrel X}
           (is0 : @isbinophrel X L) (is1 : isrngmultgt X L) (is2 : L 1 0)
           (nc : neqchoice L) (isa : isasymm L) :
  iscomprelrelfun L (fldfracgt X is is0 is1 is2 nc) (tofldfrac X is).
Proof.
  intros. intros x1 x2 l.
  assert (int := iscomptocommrngfrac
                   X (rngpossubmonoid X is1 is2) is0 is1 (fun c r => r)).
  ...
  assert (ee : Π x : X, paths (tocommrngfrac X (rngpossubmonoid X is1 is2) x)
                              (weqfldfracgt_f
                                 X is is0 is1 is2 nc (tofldfrac X is x))).
  {
    intros x.
    ...
    with (setquotpr (eqrelcommrngfrac X (rngpossubmonoid X is1 is2))
                    (weqfldfracgtint_f
                       X is0 is1 is2 nc
                       (dirprodpair x (tpair (fun a => neg (paths a 0))
                                             1 (nonzeroax X))))).
    apply (maponpaths
             (setquotpr (eqrelcommrngfrac X (rngpossubmonoid X is1 is2)))).
