...
Theorem isarchfldfrac ( X : intdom ) ( is : isdeceq X )  { R : hrel X }
        ( is0 : @isbinophrel ( rngaddabgr X ) R ) ( is1 : isrngmultgt X R )
        ( is2 : R 1 0 ) ( nc : neqchoice R )
        ( newcond : forall x : rngpossubmonoid X is1 is2 ,
                      ishinh ( total2 ( fun x' : X => R ( ( pr1 x ) * x' ) 1 ) ) )
        ( asy : isasymm R ) ( z : fldfrac X is ) :
  ishinh ( total2 ( fun x : X => fldfracgt _  is is0 is1 is2 nc ( tofldfrac X is x ) z ) ) .
Proof.
  intros.
  set ( P := fun z =>
               ishinh (
                   total2 ( fun x : X =>
                              fldfracgt _  is is0 is1 is2 nc ( tofldfrac X is x ) z ) ) ) .
  set ( P' := fun z' =>
                ishinh (
                    total2 ( fun x : X =>
                               commrngfracgt
                                 X
                                 ( rngpossubmonoid X is1 is2 ) is0 is1
                                 ( fun c r => r )
  ...
  refine ( tpair _ _ _ ) .
  refine ( tpair _ _ _ ) .  exact ( 1 : ( pr1 X ) ) . exact is2 .  simpl .
  ...
  exact ( 1 : ( pr1 X ) ) . exact is2 .  simpl .
  repeat (rewrite ( rngrunax2 X _ )) .
  change ( R ( 0%rig * ( ( pr1 x1 ) : ( rngtorig X ) ) ) x0 ) .
  rewrite ( rigmult0x X ( ( pr1 x1 ) : X ) ) . apply x0lt0 .
