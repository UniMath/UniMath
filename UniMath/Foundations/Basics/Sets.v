...
Lemma isapropsubtype { X : UU } ( A : subtype X ) ( is : Π ( x1 x2 : X ) , A x1 -> A x2 -> x1 = x2 ) : isaprop ( carrier A ) .
...
Definition iseqclass { X : UU } ( R : hrel X ) ( A : subtype X ) := dirprod ( ishinh ( carrier A ) ) ( dirprod ( Π x1 x2 : X , R x1 x2 -> A x1 -> A x2 ) ( Π x1 x2 : X, A x1 ->  A x2 -> R x1 x2 ) ).
Definition iseqclassconstr { X : UU } ( R : hrel X ) { A : subtype X } ( ax0 : ishinh ( carrier A ) ) ( ax1 : Π x1 x2 : X , R x1 x2 -> A x1 -> A x2 ) ( ax2 : Π x1 x2 : X, A x1 ->  A x2 -> R x1 x2 ) : iseqclass R A := dirprodpair ax0 ( dirprodpair ax1 ax2 ) .

Definition eqax0 { X : UU } { R : hrel X } { A : subtype X }  : iseqclass R A -> ishinh ( carrier A ) := fun is : iseqclass R A =>  pr1 is .
Definition eqax1 { X : UU } { R : hrel X } { A : subtype X } : iseqclass R A ->  Π x1 x2 : X,  R x1 x2 -> A x1 -> A x2 := fun is: iseqclass R A => pr1 ( pr2 is) .
Definition eqax2 { X : UU } { R : hrel X } { A : subtype X } : iseqclass R A ->  Π x1 x2 : X,  A x1 -> A x2 -> R x1 x2 := fun is: iseqclass R A => pr2 ( pr2 is) .
...
Lemma iseqclassresrel { X : UU } ( R : hrel X ) ( P Q : subtype X ) ( is : iseqclass R Q ) ( is' : Π x , Q x -> P x ) : iseqclass ( resrel R P ) ( fun x : P => Q ( pr1 x ) ) .
Proof . intros . split .
