...
Definition ZGuidedHomotopy {Y} {y:Y} (l:y = y) (T:Torsor ℤ) :=
  GuidedHomotopy (λ _:T,y) (λ _:T,l).
...
Definition GH_path3 {Y} {y:Y} (l:y = y) {T:Torsor ℤ} {y':Y}
           {g g':GHomotopy (λ _:T,y) (λ _:T,l) y'} (u:g = g') :
...
Definition pr22_GH {Y} {y:Y} {l:y = y} (u:GH l)
     := pr2 (pr2_GH l u)
     : GHomotopy (λ _:pr1_GH u, y) (λ _:pr1_GH u, l) (pr12_GH u).
...
Local Definition irr {Y} {y:Y} (l:y = y) (T:Torsor ℤ) := proofirrGuidedHomotopy T (λ _:T,y) (λ _:T,l).

Local Definition sec {Y} {y:Y} (l:y = y) (T:Torsor ℤ) := makeGuidedHomotopy2 T (λ _:T,y) (λ _:T,l).
...
Proof. intros.
       ...
       refine (maponpaths (maponpaths pr12_GH)
                  (homotinvweqweq_GH_comp
                     T0
                     (makeGuidedHomotopy (λ _:T0, y) (λ _:T0, l) t0 (idpath y)))
                  @ _).
       refine (pr12_pair_path_in2
                 l T0
                 (irr l T0 (sec l T0)
                      (makeGuidedHomotopy (fun _ : T0 => y) (λ _:T0, l) t0 (idpath y)))
                 @ _).
       unfold sec.
       change (makeGuidedHomotopy (fun _ : T0 => y) (λ _:T0, l) t0 (idpath y))
       with (sec l T0).
...
Proof. intros. assert (p := pr1_GH_weq_compute l).
       ...
       { intermediate_path (maponpaths (circle_map l) circle_loop @ idpath y).
         ...
         { apply pathsinv0. rewrite pathsinv0inv0.
           exact (maponpaths (fun r => maponpaths (circle_map l) circle_loop @ r) p). } }
       { exact (maponpaths (fun r => r @ l) p). } Defined.
