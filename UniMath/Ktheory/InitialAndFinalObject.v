...
Definition theTerminalIdentity {C:precategory} {z:TerminalObject C}
           (f:theTerminalObject z → theTerminalObject z) : f = identity _.
Proof. intros. apply isapropifcontr. apply theTerminalProperty. Defined.
...
Definition squashTerminalObject (C:precategory) := ∥ TerminalObject C ∥.

Definition squashTerminalObjectProp (C:precategory) :=
  hProppair (squashTerminalObject C) (propproperty _).
...
Arguments theInitialObject [C] i.
Arguments theInitialProperty [C] i x.

Definition theInitialIdentity {C:precategory} (z:InitialObject C)
           (f:theInitialObject z → theInitialObject z) : f = identity _.
Proof. intros. apply isapropifcontr. apply theInitialProperty. Defined.
...
Definition pack {C:precategory} : InitialObject C -> InitialObject_total C
  := fun Y => (theInitialObject Y,,theInitialProperty Y).
...
Definition squashInitialObject (C:precategory) := ∥ InitialObject C ∥.

Definition squashInitialObjectProp (C:precategory) :=
  hProppair (squashInitialObject C) (propproperty _).
