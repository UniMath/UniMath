...
Require Import UniMath.Preliminaries.Utilities UniMath.Ktheory.Precategories.
...
Proof. apply impred. intro a. apply impred. intro b. apply isapropisweq. Qed.
...
Definition path_pregroupoid (X:UU) : isofhlevel 3 X -> Precategory.
  ...
  intros iobj.
  unshelve refine (Precategories.makePrecategory X (fun x y => x = y) _ _ _ _ _ _).
  { reflexivity. }
  { intros. exact (f @ g). }
