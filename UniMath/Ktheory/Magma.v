...
Local Notation "g ∘ f" := (binopfuncomp f g) (only parsing).
...
Definition funEquality G H (p q : Hom G H)
           (v : pr1 p = pr1 q) : p = q.
Proof.
  revert p q v; intros [p i] [q j] v. simpl in v. destruct v.
  destruct (pr1 (isapropisbinopfun p i j)). reflexivity.
Qed.
(** the trivial magma *)
Definition zero : magma.
  exists unitset. exact (λ _ _, tt).
Defined.
(** product of magmas
...
Module Product.
  ...
  Proof.
    intros. apply (impred 2); intros i. apply pr2.
  Qed.
  (** construction of the product *)
  Definition make {I} (X:I->magma) : magma.
  Proof.
    intros.
    exists ((∏ i, X i),,i1 X). exact (λ v w i, v i * w i).
  Defined.
  (** the projection maps *)
  Definition Proj {I} (X:I->setwithbinop) : ∏ i:I, Hom (make X) (X i).
  Proof.
    intros. exists (λ y, y i). intros a b. reflexivity.
  Defined.
  (** the universal map *)
