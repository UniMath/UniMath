...
Proof.
  ...
  - assert ( is1 : iscontr ( hfiber ( stntonat n ) x ) ) .
    ...
    + assumption .
    + assumption .
  - assert ( is1 : neg ( hfiber ( stntonat ( S n ) ) ( di i x ) ) ) .
...
Proof.
  set ( f := λ j : stn 2, match ( isdeceqnat j 0 ) with ii1 _ => false | ii2 _ => true end ) .
  set ( g := λ b : bool, match b with false => stnpair 2 0 ( idpath true ) | true => stnpair 2 1 ( idpath true ) end ) .
  split with f .
...
Proof.
  ...
  unfold funcomp. apply maponpaths. apply subtypeEquality_prop. induction e. apply idpath.
Defined.
...
Proof.
  ...
  { apply map_on_two_paths.
    { apply stnsum_eq; intro i. induction i as [i I]. apply maponpaths. apply subtypeEquality_prop.
      ...
      { unfold stntonat; simpl; repeat rewrite transport_stn; simpl.
        induction (natlthorgeh i j) as [a|b]. { simpl. apply idpath. } { simpl. contradicts R (natlehneggth b). }}
      { unfold stntonat; simpl; repeat rewrite transport_stn; simpl.
        induction (natlthorgeh i j) as [V|V]. { simpl. contradicts I (natlehneggth R). } { simpl. apply idpath. }}}
  ...
  apply maponpaths. rewrite transport_stn; simpl. apply subtypeEquality_prop. apply idpath.
Defined.
...
Proof.
  ...
  - assert (t : stnsum m1 + j < stnsum m1 + m (i,,I)).
    ...
    + unfold funcomp. apply maponpaths. apply subtypeEquality_prop. simpl. apply idpath.
Defined.
...
Proof .
  ...
  - intro .
    ...
    set ( fl := λ i : stn 1, f ( g ( ii1 i ) ) ) .
    set ( fh := λ i : stn n, f ( g ( ii2 i ) ) ) .
    ...
    + split with ( S x0 ) .
      assert ( wi : ( hfiber fl true ) ≃ ( stn 1 ) ) .
...
Proof.
  ...
  - intro m .
    ...
    assert ( w2 : weq ( stn ( S n ) -> stn m ) ( ((stn 1) ⨿ (stn n)) -> stn m ) ) by apply ( weqbfun _ w1  ) .
    set ( w3 := weqcomp w2 ( weqfunfromcoprodtoprod ( stn 1 ) ( stn n ) ( stn m ) ) ) .
    set ( w4 := weqcomp w3 ( weqdirprodf ( weqfunfromcontr ( stn m ) iscontrstn1 ) ( IHn m ) ) ) .
...
Proof.
  ...
  - intro.
    apply 1 .
  - intro f .
    apply (  ( IHn ( λ i : stn n, f ( dni lastelement i ) ) ) * f lastelement ) .
Defined .
...
Proof .
  ...
  - apply ( ischoicebaseweqf ( weqdnicoprod n lastelement ) ( ischoicebasecoprod IHn ischoicebaseunit ) ) .
Defined .
...
Proof.
  unfold neg.
  intro.
  assert (lp: stn (S n)) by apply lastelement.
  intro X.
...
Proof. intro.
       ...
       - intro n'. destruct n' as [ | n' ] .
         ...
         + intro X. apply maponpaths. apply IHn. apply weqcutforstn. assumption.
Defined.
...
Proof .
  intros .
  assert ( lg : ( ∏ n : nat , natleh n ( S n' ) -> F n ) <-> dirprod ( ∏ n : nat , natleh n n' -> F n ) ( F ( S n' ) ) ) .
  { split .
    - intro f.
      apply ( dirprodpair ( λ n, λ l, ( f n ( natlehtolehs _ _ l ) ) ) ( f ( S n' ) ( isreflnatleh _ ) ) ) .
    - intro d2 .
      intro n . intro l .
  ...
  assert ( is2 : isaprop ( dirprod ( ∏ n : nat , natleh n n' -> F n ) ( F ( S n' ) ) ) ) .
...
Proof .
  intros .
  assert ( lg : hexists ( λ n : nat, dirprod ( natleh n ( S n' ) ) ( P n ) )  <-> hdisj ( hexists ( λ n : nat, ( natleh n n' ) × ( P n ) ) )  ( P ( S n' ) )  ) .
  { split .
    - apply hinhfun .
      intro t2 .
      destruct t2 as [ n d2 ] .
...
Proof.
  intros. unfold concatenate'. rewrite weqfromcoprodofstn_invmap_r0. apply idpath.
Defined.
...
Proof.
  ...
  - rewrite stnsum_step.
    ...
    + apply maponpaths.
      ...
      * unfold funcomp. apply stnsum_eq; intros i. rewrite replace_dni_last. apply idpath.
