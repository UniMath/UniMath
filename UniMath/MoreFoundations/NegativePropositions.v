...
Theorem isaproppathsfromisolated_ne (X : UU) (x : X) (neq_x : neqPred x)
        (isi : isisolated_ne X x neq_x) (y : X)
  : isaprop (x = y).
Proof.
  (* we could follow the proof of isaproppathsfromisolated here, but we try a
     different way *)
  intros. unfold isisolated_ne in isi. apply invproofirrelevance; intros m n.
  set (Q y := (x = y) ⨿ (neq_x y)).
  assert (a := (transport_section isi m) @ !(transport_section isi n)).
  induction (isi x) as [j|k].
...
Definition maponcomplincl_ne {X Y : UU} (f : X -> Y) (isin : isincl f) (x : X)
...
Proof.
  ...
  exact (f x',,neg_to_negProp (nP := neq_fx (f x'))
           (negf (invmaponpathsincl _ isin x x') (negProp_to_neg neqx))).
Defined.
...
Definition invrecompl_ne (X : UU) (x : X) (neq_x : neqPred x)
           (isi : isisolated X x) : X -> compl_ne X x neq_x ⨿ unit.
Proof.
  intros y. induction (isi y) as [k|k].
  - exact (ii2 tt).
  - exact (ii1 (make_compl_ne X x neq_x y (neg_to_negProp k))).
Defined.

Theorem isweqrecompl_ne (X : UU) (x : X) (isi : isisolated X x)
        (neq_x : neqPred x) : isweq (recompl_ne _ x neq_x).
Proof.
  (* does not use [funextemptyAxiom] *)
  intros.
  set (f := recompl_ne X x neq_x). set (g := invrecompl_ne X x neq_x isi).
  refine (isweq_iso f g _ _).
  {intro u. induction (isi (f u)) as [ eq | ne ].
   - induction u as [ c | u].
...
Proof.
  ...
  unfold neqPred,negProp in neq_x; unfold isisolated in isi.
  apply (iscontrweqb (weqtotal2overcoprod _)). induction (isi y) as [eq|ne].
  {induction eq. refine (iscontrweqf (weqii2withneg _ _) _).
   ...
   {change x with (f (ii2 tt)). simple refine ((_,,_),,_).
    ...
    {intro w. induction w as [t e]. unfold f in *; simpl in *. induction t.
     apply maponpaths. apply isaproppathsfromisolated. exact isi. }}}
...
Definition weqrecompl_ne (X : UU) (x : X) (isi : isisolated X x)
           (neq_x : neqPred x) : compl_ne X x neq_x ⨿ unit ≃ X
  := make_weq _ (isweqrecompl_ne X x isi neq_x).
...
Proof.
  ...
  apply (isweqhomot (weqrecompl_ne X x isi neq_x
