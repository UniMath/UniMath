(** The second layer of the construction of the bicategory of strict pseudofunctors consists of three parts.
    Third part: we add a 2-cell witnessing the strict preservation of composition.
 *)
...
Section StrictCompositor.
  Variable (C D : bicat).

  Definition strict_compositor_disp_cat_data
  ...
  Proof.
    ...
    - exact (λ F G Fcomp Gcomp η,
             ∏ (X Y Z : C) (f : X --> Y) (g : Y --> Z),
             (ηobj η X ◃ idtoiso_2_1 _ _ (Gcomp X Y Z f g))
               • ηmor η (f · g)
             =
             (lassociator (ηobj η X) (Fmor G f) (Fmor G g))
               ...
               • (idtoiso_2_1 _ _ (Fcomp X Y Z f g) ▹ ηobj η Z)).
  Defined.

  Definition strict_compositor_disp_cat_id_comp
    : disp_cat_id_comp (map1cells C D) strict_compositor_disp_cat_data.
  Proof.
    split.
  ...
  Definition strict_compositor_disp_cat
    : disp_bicat (map1cells C D).
  Proof.
    use disp_cell_unit_bicat.
    use tpair.
    - exact strict_compositor_disp_cat_data.
    - exact strict_compositor_disp_cat_id_comp.
  Defined.

  Definition strict_compositor_is_disp_univalent_2_1
    : disp_univalent_2_1 strict_compositor_disp_cat.
  Proof.
  ...
  Definition strict_compositor_is_disp_univalent_2_0
             (HD_2_1 : is_univalent_2_1 D)
    : disp_univalent_2_0 strict_compositor_disp_cat.
  Proof.
    ...
    - intros F F₂ F₂' η ; cbn in *.
      ...
      rewrite !vassocr in η₁.
      assert (pr1 (idtoiso_2_1 _ _ (F₂ X Y Z f g))
              = pr1 (idtoiso_2_1 _ _ (F₂' X Y Z f g))) as H.
      {
      ...
      assert (isotoid_2_1 HD_2_1 (idtoiso_2_1 _ _ (F₂ X Y Z f g))
              =
      ...
      refine (!_ @ H' @ _).
      + apply (homotinvweqweq (make_weq (idtoiso_2_1 _ _)
                                        (HD_2_1 _ _ _ _))).
      + apply (homotinvweqweq (make_weq (idtoiso_2_1 _ _)
                                        (HD_2_1 _ _ _ _))).
  Defined.

  Definition strict_compositor_is_disp_univalent_2
             (HD_2_1 : is_univalent_2_1 D)
    : disp_univalent_2 strict_compositor_disp_cat.
  Proof.
    split.
    - apply strict_compositor_is_disp_univalent_2_0; assumption.
    - exact strict_compositor_is_disp_univalent_2_1.
  Defined.
End StrictCompositor.
