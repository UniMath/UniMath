...
(*
   produce an adjoint equivalence from an equivalence
   see
   - Theorem 4.2.3 of HoTT book (https://hottheory.files.wordpress.com/2013/03/hott-online-611-ga1a258c.pdf)
   - http://ncatlab.org/nlab/show/adjoint+equivalence

*)
Section equivalence.

Variables A C : precategory.
Variable hsA : has_homsets A.
Variable hsC : has_homsets C.
Variable L : functor A C.

Definition is_equivalence :=
  Σ R : functor C A,
        iso (C:= [A, A, hsA]) (functor_identity _ )(functor_composite L R)
            ×
        iso (C:= [C,C,hsC]) (functor_composite R L) (functor_identity _ ).

Variable R : is_equivalence.

Definition adj_equivalence_from_equivalence : adj_equivalence_of_precats L.
Proof.
  refine (tpair _ _ _ ).
  - refine (tpair _ _ _ ).
    + apply (pr1 R).
