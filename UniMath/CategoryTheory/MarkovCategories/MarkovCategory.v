...
(* TODO prove pairingproj lemmas without determinims *)
Section PairingProj.
  Context {C : markov_category}.
  ...
  Lemma pp_tensor {x1 x2 y1 y2 : C} (f : x1 --> y1) (g : x2 --> y2) :
    f #⊗ g = ⟨proj1 · f, proj2 · g⟩.
  Proof.
    rewrite <- pairing_tensor.
    rewrite <- pp_id, id_left.
    reflexivity.
  Qed.

  Lemma pp_braiding (x y : C) :
    sym_mon_braiding C x y = ⟨proj2, proj1⟩.
  Proof.
    rewrite pairing_eq.
    rewrite <- copy_tensor.
    assert(proj_inner_swap : inner_swap _ x x y y · proj2 #⊗ proj1 = proj2 #⊗ proj1 · sym_mon_braiding C _ _).
    {
      unfold proj1, proj2.
      rewrite !tensor_comp_mor.
      rewrite !assoc.
