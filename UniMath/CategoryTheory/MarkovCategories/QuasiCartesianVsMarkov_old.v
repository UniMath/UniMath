...
Proposition markov_to_quasicartesian_inv' (C : markov_category) : 
  quasicartesian_to_markov (markov_to_quasicartesian C) = C.
Proof.
  pose(CC := C).
  destruct C as [[[Cmon [s slaws]] markov] l].
  use subtypePath. { intros cc. apply isaprop_markov_category_laws. } cbn.
  apply reassoc_eq. unfold total2asstor. cbn.
  apply reassoc_eq. unfold total2asstor. cbn.   
  apply maponpaths.
  ...
  (* use total2_paths_f. { 
    repeat (apply funextsec2; intros). cbn.
    (* Check pairing_proj_whisker_l.  *)
    etrans. {
      refine (!(@pairing_proj_whisker_l CC _ _ _ _ )).
    }
    reflexivity.
  }   *)
