...
Section Quasicartesian.
  ...
  Context (pairing_id : ∏ (x y : C), identity (x ⊗ y) = ⟨proj1, proj2⟩).
  ...
  Lemma det_proj (x y z : C) (f : x --> y ⊗ z) :
  ...
  Notation "f #⊗ g" := (⟨ proj1 · f, proj2 · g ⟩).

  (* Linear contexts can be undone *)
  (* First step: derive the other assoc' by invertibility *)

  (* Now: Every linear coherence map is a composite of
      - tensors
      - swaps
      - associators or their inverses
    So really, we gave enough naturality laws
  *)
  Proposition cancel_assoc {a x y z : C} (f g : a --> (x ⊗ y) ⊗ z) :
    f · ⟨ proj1 · proj1, ⟨ proj1 · proj2, proj2 ⟩⟩ = g · ⟨ proj1 · proj1, ⟨ proj1 · proj2, proj2 ⟩⟩
    -> f = g.
  Proof.
    intros e.
    pose(inv := ⟨ ⟨ proj1, proj2 · proj1 ⟩, proj2 · proj2 ⟩ :> x ⊗ (y ⊗ z) --> (x ⊗ y) ⊗ z).
    assert(r : identity _ = ⟨ proj1 · proj1, ⟨ proj1 · proj2, proj2 ⟩⟩ · inv). {
      unfold inv. do 2 qcart_eta. }
    rewrite <- (id_right g), <- (id_right f).
    rewrite !r, !assoc.
    rewrite e.
    reflexivity.
  Qed.

  Lemma inner_swap_nat {a x1 y1 x2 y2 : C} (f1 : a --> x1) (f2 : a --> x2) (g1 : a --> y1) (g2 : a --> y2) :
    ⟨ ⟨ f1, g1 ⟩, ⟨ f2, g2 ⟩ ⟩ · ⟨ ⟨ proj1 · proj1, proj2 · proj1 ⟩, ⟨ proj1 · proj2, proj2 · proj2 ⟩ ⟩ =
    ⟨ ⟨ f1, f2 ⟩, ⟨ g1, g2 ⟩ ⟩.
  Proof.
    apply cancel_assoc.
    rewrite pairing_assoc.
