...
Section CodomainStructure.
  ...
  Section LeftAdjointSubst.
    ...
    Definition comp_functor_counit
      : fiber_functor_from_cleaving D HD f ∙ comp_functor ⟹ functor_identity _.
    Proof.
      use make_nat_trans.
      - simple refine (λ g, _ ,, _).
        + apply PullbackPr1.
  ...
  Proof.
    simple refine (comp_functor f ,, (comp_functor_unit f ,, comp_functor_counit f) ,, (_ ,, _)).
    all:apply TODO.
    (*
  ...
  Proof.
    ...
    - simpl.
      ...
      pose (counit_from_right_adjoint (is_right_adjoint_cod_fiber_functor h)
         (fiber_functor_from_cleaving D HD g
            (left_adjoint (is_right_adjoint_cod_fiber_functor f) φ))).
      cbn in p0.
      cbn.
      simpl.
      use is_z_iso_fiber_from_is_z_iso_disp.
