...
Section elems_slice_equiv.
  ...
  Definition pshf_to_slice_ob_isnat (X Y : pshf (el P)) (f : X --> Y) :
    is_nat_trans (pshf_to_slice_ob_funct_data X) (pshf_to_slice_ob_funct_data Y) (pshf_to_slice_ob_nat X Y f).
    simpl.
    intros X Y f c c' g.
    apply funextsec; intro p.
    apply pair_path_in2.
  ...
  Definition pshf_to_slice_mor (X Y : pshf (el P)) (f : X --> Y) :
    ...
    simpl.
    intros X Y f.
    exists (pshf_to_slice_ob_nat X Y f ,, pshf_to_slice_ob_isnat X Y f).
  ...
  Proof.
    intros [[[X Xmor] [Xid Xcomp]] [Xnat Xisnat]] [[[Y Ymor] [Yid Ycomp]] [Ynat Yisnat]] f e.
    unfold slice_to_pshf_ob_ob. unfold sl_nat. simpl.
  Admitted.
  ...
  Proof.
    intros X Y f.
    exists (slice_to_pshf_ob_nat X Y f).
  Admitted.
