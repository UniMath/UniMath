...
Require Import UniMath.CategoryTheory.Monoidal.MonoidalTransformationsWhiskered.
...
Section MonoidalMonad.
  ...
  Definition LMoMonadMult {M : monoidal C} {T : Monad C} (MT : fmonoidal_lax M M T) : UU
    := nmonoidal (fmonoidal_composition_lax MT MT) MT (pr211 T).

  Definition LMoMonadUnit {M : monoidal C} {T : Monad C} (MT : fmonoidal_lax M M T) : UU
    := nmonoidal (fmonoidal_identity_lax M) MT (pr21 T).

  Definition LMoMonad_data (M : monoidal C) (T : Monad C)  : UU
    := ∑ MT : fmonoidal_lax M M T, LMoMonadMult MT × LMoMonadUnit MT.

  Definition LMoMonadDataToFmonoidal {M : monoidal C} {T : Monad C} (MT : LMoMonad_data M T) : fmonoidal_lax M M T := pr1 MT.
  Coercion LMoMonadDataToFmonoidal : LMoMonad_data >-> fmonoidal_lax.

  Definition LMoMonad_laws {M : monoidal C} {T : Monad C} (LM : LMoMonad_data M T) : UU
    :=
...
Section MonoidalMonad.
  ...
  Definition MoMonadMult {M : monoidal C} {T : Monad C} (MT : fmonoidal M M T) : UU
    := nmonoidal (fmonoidal_composition MT MT) MT (pr211 T).

  Definition MoMonadUnit {M : monoidal C} {T : Monad C} (MT : fmonoidal M M T) : UU
    := nmonoidal (fmonoidal_identity M) MT (pr21 T).

  Definition MoMonad (M : monoidal C) (T : Monad C)  : UU
    := ∑ MT : fmonoidal M M T, MoMonadMult MT × MoMonadUnit MT.

  Definition MoMonadToFmonoidal {M : monoidal C} {T : Monad C} (MT : MoMonad M T) : fmonoidal_lax M M T := pr1 MT.
  Coercion MoMonadToFmonoidal : MoMonad >-> fmonoidal_lax.
...
Section MonoidalAlgebras.
  ...
  Local Notation "pt_{ TM }" := (fmonoidal_preservestensordata (TM : fmonoidal M M (pr11 T))) (at level 31).

  Context {pts : preserves_tensor_strongly (pt_{TM})}.
