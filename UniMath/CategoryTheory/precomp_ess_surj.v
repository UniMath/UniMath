...
Local Definition X (b : B) := total2 (
 fun ck :
  total2 (λ c : C,
                ∏ a : A,
                     z_iso (H a) b -> z_iso (F a) c) =>
    ∏ t t' : total2 (λ a : A, z_iso (H a) b),
          ∏ f : pr1 t --> pr1 t',
             (#H f · pr2 t' = pr2 t ->
...
Lemma X_aux_type_center_of_contr_proof (b : B) (anot : A) (hnot : z_iso (H anot) b) :
  ∏ (t t' : total2 (λ a :  A, z_iso (H a) b))
    (f : pr1 t --> pr1 t'),
  #H f· pr2 t' = pr2 t ->
  #F f·
  #F (fH^-1 (pr2 t'· inv_from_z_iso hnot)) =
  #F (fH^-1 (pr2 t· inv_from_z_iso hnot)).
Proof.
  intros t t' f.
  destruct t as [a h].
...
Definition X_aux_type_center_of_contr (b : B)
    (anot : A)(hnot : z_iso (H anot) b) : X b.
Proof.
  set (cnot := F anot).
  set (g := fun (a : A)(h : z_iso (H a) b) =>
              (fH^-i (z_iso_comp h (z_iso_inv_from_z_iso hnot)))).
  set (knot := fun (a : A)(h : z_iso (H a) b) =>
                    functor_on_z_iso F (g a h)).
  simpl in *.
...
Lemma X_aux_type_contr_eq (b : B) (anot : A) (hnot : z_iso (H anot) b) :
...
Proof.
  ...
  { set (w := isotoid _ Ccat ((pr2 (pr1 t)) anot hnot) :
    ...
    set (gah := fH^-i (z_iso_comp h (z_iso_inv_from_z_iso hnot))).
    set (qhelp := q1 (tpair _ a h)(tpair _ anot hnot) gah).
    simpl in *.

    assert (feedtoqhelp :
             #H (fH^-1 (h· inv_from_z_iso hnot))· hnot = h).
    { inv_functor fH a anot.
      rewrite <- assoc.
    ...
    simpl in *.
    intermediate_path (z_iso_comp  (functor_on_z_iso F
...
Local Definition k (b : B) :
     ∏ a : A, z_iso (H a) b -> z_iso (F a) (Go b) :=
...
Lemma Y_inhab_proof (b b' : B) (f : b --> b') (a0 : A) (h0 : z_iso (H a0) b)
    (a0' : A) (h0' : z_iso (H a0') b') :
  ∏ (a : A) (h : z_iso (H a) b) (a' : A) (h' : z_iso (H a') b')
    (l : a --> a'),
  #H l· h' = h· f ->
  #F l· k b' a' h' =
    k b a h· ((inv_from_z_iso (k b a0 h0)·
  #F (fH^-1 ((h0· f)· inv_from_z_iso h0')))· k b' a0' h0').
Proof.
  ...
  set (hfh := h0 · f · inv_from_z_iso h0').
  set (l0 := fH^-1 hfh).
  set (g0 := inv_from_z_iso (k b a0 h0) · #F l0  · k b' a0' h0').
  ...
  { rewrite functor_comp .
    ...
    unfold hfh.
    intermediate_path (h0 · f · (inv_from_z_iso h0' · h0') · inv_from_z_iso h').
    { repeat rewrite assoc; apply idpath. }
    rewrite z_iso_after_z_iso_inv, id_right, functor_comp.
    inv_functor fH a0 a.
    repeat rewrite <- assoc.
    apply maponpaths, pathsinv0, z_iso_inv_on_right.
    rewrite assoc.
    apply z_iso_inv_on_left, pathsinv0, alpha.
  }
  assert (star5 : inv_from_z_iso m · l0 = l · inv_from_z_iso m').
  ...
  clear sssss.
  unfold g0.
  assert (sss'' : k b a h · inv_from_z_iso (k b a0 h0) =
                    inv_from_z_iso (functor_on_z_iso F m)).
  { apply pathsinv0, z_iso_inv_on_left, pathsinv0.
    apply z_iso_inv_on_right.
    unfold m; simpl.
  ...
  rewrite functor_comp, functor_on_inv_from_z_iso.
  assert (star4 :
        inv_from_z_iso (functor_on_z_iso F m')· k b' a0' h0'
           = k b' a' h' ).
  { apply z_iso_inv_on_right.
...
Definition Y_inhab (b b' : B) (f : b --> b')
      (a0 : A) (h0 : z_iso (H a0) b) (a0' : A) (h0' : z_iso (H a0') b') : Y f.
Proof.
  set (hfh := h0 · f · inv_from_z_iso h0').
  set (l0 := fH^-1 hfh).
  set (g0 := inv_from_z_iso (k b a0 h0) · #F l0  · k b' a0' h0').
...
Proof.
  ...
  - unfold functor_idax. simpl.
    intro b.
    assert (PR2 : ∏ (a : A) (h : z_iso (H a) b) (a' : A)
                    (h' : z_iso (H a') b)
                    (l : a --> a'),
               #H l· h' = h· identity b ->
  ...
  - (** composition *)
    ...
    intros [a0'' h0''].
    simpl; clear HHHH.
    set (l0 := fH^-1 (h0 · f · inv_from_z_iso h0')).
    set (l0' := fH^-1 (h0' · f' · inv_from_z_iso h0'')).
    set (l0'' := fH^-1 (h0 · (f· f') · inv_from_z_iso h0'')).
    assert (L : l0 · l0' = l0'').
    { apply (invmaponpathsweq (weq_from_fully_faithful fH a0 a0'')).
      ...
      unfold l0.
      inv_functor fH a0 a0'.
      intermediate_path (h0 · f · (inv_from_z_iso h0' · h0') · f' · inv_from_z_iso h0'').
      { repeat rewrite assoc; apply idpath. }
      rewrite z_iso_after_z_iso_inv, id_right.
      unfold l0''.
      inv_functor fH a0 a0''.
    ...
    assert (PR2 : ∏ (a : A) (h : z_iso (H a) b)(a' : A)
          (h' : z_iso (H a') b') (l : a --> a'),
           #H l· h' = h· f ->
           #F l· k b' a' h' =
             k b a h· ((inv_from_z_iso (k b a0 h0)· #F l0)· k b' a0' h0') ).
    { intros a h a' h' l.
      intro alpha.
      set (m := fH^-i (z_iso_comp h0 (z_iso_inv_from_z_iso h))).
      set (m' := fH^-i (z_iso_comp h0' (z_iso_inv_from_z_iso h'))).
      ...
      rewrite <- functor_on_inv_from_z_iso.
      rewrite <- functor_comp.
      rewrite star5, functor_comp, functor_on_inv_from_z_iso.
      clear star5.
      assert (star4 :
               inv_from_z_iso (functor_on_z_iso F m')· k b' a0' h0'
               = k b' a' h' ).
    ...
    assert (HGf : G f = inv_from_z_iso (k b a0 h0) · #F l0 · k b' a0' h0').
    { set (Gbrtilde :=
           tpair _ (inv_from_z_iso (k b a0 h0) · #F l0 · k b' a0' h0') PR2 : Y f).
    ...
    { intros a' h' a'' h'' l'.
      ...
      assert (sss'' : k b' a' h' · inv_from_z_iso (k b' a0' h0') =
                        inv_from_z_iso (functor_on_z_iso F m)).
      { apply pathsinv0, z_iso_inv_on_left, pathsinv0, z_iso_inv_on_right.
        unfold m; simpl;
          apply pathsinv0, sss'.
      }
      repeat rewrite assoc.
    ...
    assert (HGf' : G f' = inv_from_z_iso (k b' a0' h0') · #F l0' · k b'' a0'' h0'').
    { set (Gbrtilde :=
             tpair _ (inv_from_z_iso (k b' a0' h0') · #F l0' · k b'' a0'' h0'') PR2 :
                     Y f').
      set (H' := pr2 (Y_iscontr b' b'' f') Gbrtilde).
    ...
    { intros a h a'' h'' l.
      ...
      { rewrite functor_comp.
        ...
        unfold l0''.
        inv_functor fH a0 a0''.
        intermediate_path (h0 · (f · f') · (inv_from_z_iso h0'' · h0'') · inv_from_z_iso h'').
        { repeat rewrite assoc; apply idpath. }
        rewrite z_iso_after_z_iso_inv, id_right, functor_comp.
        inv_functor fH a0 a.
        repeat rewrite <- assoc.
        apply maponpaths, pathsinv0, z_iso_inv_on_right.
        repeat rewrite assoc.
        apply z_iso_inv_on_left, pathsinv0.
        repeat rewrite <- assoc.
        apply alpha.
      }
      assert (star5 : inv_from_z_iso m · l0'' = l · inv_from_z_iso m').
    ...
    assert (HGff' : G (f · f') =
                      inv_from_z_iso (k b a0 h0) · #F l0'' · k b'' a0'' h0'').
    { set (Gbrtilde :=
           tpair _ (inv_from_z_iso (k b a0 h0) · #F l0'' · k b'' a0'' h0'') PR2 :
               Y (f · f')).
    ...
    clear PR2.
    rewrite HGf, HGf'.
    intermediate_path (inv_from_z_iso (k b a0 h0)· #F l0· (k b' a0' h0'·
                       inv_from_z_iso (k b' a0' h0'))· #F l0'· k b'' a0'' h0'').
...
Lemma qF (a0 : A) :
  ∏ (t t' : total2 (λ a :  A, z_iso (H a) (H a0)))
    (f : pr1 t --> pr1 t'),
  #H f· pr2 t' = pr2 t ->
  #F f· #F (fH^-1 (pr2 t')) =
  #F (fH^-1  (pr2 t)).
...
Definition kFa (a0 : A) : ∏ a : A,
  z_iso (H a) (H a0) -> z_iso (F a) (F a0) :=
 fun (a : A) (h : z_iso (H a) (H a0)) =>
   functor_on_z_iso F
...
Proof.
  ...
  rewrite idtoiso_inv.
  rewrite <- assoc.
  assert (PSIf : ∏ (a : A) (h : z_iso (H a) (H a0)) (a' : A)
     (h' : z_iso (H a') (H a0')) (l : a --> a'),
         #H l· h' = h· #H f ->
         #F l· k (H a0') a' h' =
         k (H a0) a h·
         ((idtoiso (phi a0)· #F f)· inv_from_z_iso (idtoiso (phi a0')))).
    { intros a h a' h' l alpha.
      rewrite assoc.
      apply z_iso_inv_on_left.
      unfold phi.
    ...
    set (Ybla := tpair _ (idtoiso (phi a0) · #F f · inv_from_z_iso (idtoiso (phi a0')))
                    PSIf : Y (#H f)).
