...
VFILES:=Foundations/funextfun.v\
  Foundations/hProp.v\
  Foundations/hSet.v\
  Foundations/uu0.v\
  Foundations/uuu.v\
  Rezk/auxiliary_lemmas_HoTT.v\
...
VOFILES2:=$(patsubst Rezk/%,%,$(filter Rezk/%,$(VOFILES)))
VOFILES0:=$(filter-out Foundations/% Rezk/%,$(VOFILES))
...
install:
	...
	(cd Rezk; for i in $(VOFILES2); do \
	 install -d `dirname $(COQLIB)/user-contrib/Rezk/$$i`; \
	 install $$i $(COQLIB)/user-contrib/Rezk/$$i; \
	 done)
