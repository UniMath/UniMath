...
Inductive paths (T:UU): T-> T -> UU := idpath: forall t:T, paths T t t.
...
Proof. intros. simpl. induction e. apply (idtoid1 _ _ (fun x:X => x) t). Defined. 
...
Proof. intros. induction e. rename t into x. change (pathscomp0 X x x (f x) (idpath X x) (pathsinv0 X (f x) x (h x))) with (pathsinv0 X (f x) x (h x)). assert (ee: paths _  (maponpaths X X f x x (idpath X x)) (idpath _ (f x))). apply idtoid1. 
...
Proof. intros. induction e. rename t into x. split with (fun p: P x => p). simpl. split with (fun p: P x => idpath _ _). unfold maponpaths. simpl. apply (fun pp: P x => idpath _ _ ). Defined. 
...
assert (egf: forall yxe:_, paths _ (gg (ff yxe)) yxe). intro.  unfold gg. unfold tococonusf. unfold ff. unfold fromcoconusf.  simpl.   destruct yxe. destruct x. simpl.  set (s1:=constr1 Y  (fun y:Y => (hfiber X Y f y)) _ _ x).  simpl in s1. set (s2:=pr21 _ _ (pr22 _ _ s1) (hfiberpair X Y f (f t0) t0 (idpath Y (f t0)))). simpl in s2. 
...
assert (egf: forall yee: hfiber _ _ f y, paths _ (gg (ff yee)) yee). intro.  induction yee.  destruct x. simpl. rename t into x.
